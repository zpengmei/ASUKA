...
struct QmcWorkspaceU64 {
  ...
  QmcWorkspaceU64(int max_n_, int max_m_) : max_n(std::max(0, max_n_)), max_m(std::max(0, max_m_)) {
    if (max_n <= 0) {
    ...
    ws = guga_qmc_workspace_u64_create(max_n, max_m);
    if (!ws) {
      throw std::runtime_error("failed to create QmcWorkspaceU64");
    }
  }
  ...
  QmcWorkspaceU64(QmcWorkspaceU64&& other) noexcept { *this = std::move(other); }
  QmcWorkspaceU64& operator=(QmcWorkspaceU64&& other) noexcept {
    if (this != &other) {
      release();
      max_n = other.max_n;
      max_m = other.max_m;
  ...
  void release() noexcept {
    if (ws) {
      guga_qmc_workspace_u64_destroy(ws);
    }
    ws = nullptr;
  }

  void coalesce_coo_u64_f64_inplace_device(
      py::object key_in,
      py::object val_in,
      py::object key_out,
      py::object val_out,
      ...
      uint64_t stream,
      bool sync) const {
    if (!ws) throw std::runtime_error("QmcWorkspaceU64 has been released");
    if (threads <= 0 || threads > 1024) {
      throw std::invalid_argument("threads must be in 1..1024");
    }
    n = int(n);
    ...
    if (key_in.is_none() || val_in.is_none() || key_out.is_none() || val_out.is_none() || out_nnz.is_none()) {
      throw std::invalid_argument("key_in/val_in/key_out/val_out/out_nnz must be device arrays (cannot be None)");
    }

    CudaArrayView key_in_dev = cuda_array_view_from_object(key_in, "key_in");
    CudaArrayView val_in_dev = cuda_array_view_from_object(val_in, "val_in");
    CudaArrayView key_out_dev = cuda_array_view_from_object(key_out, "key_out");
    ...
    require_typestr(key_in_dev, "key_in", "<u8");
    require_typestr(val_in_dev, "val_in", "<f8");
    require_typestr(key_out_dev, "key_out", "<u8");
    require_typestr(val_out_dev, "val_out", "<f8");
    require_typestr(nnz_dev, "out_nnz", "<i4");

    if (key_out_dev.read_only || val_out_dev.read_only || nnz_dev.read_only) {
      throw std::invalid_argument("key_out/val_out/out_nnz must be writable device arrays");
    }

    if (key_in_dev.shape.size() != 1 || val_in_dev.shape.size() != 1) {
      throw std::invalid_argument("key_in and val_in must be 1D device arrays");
    }
    if (key_in_dev.shape[0] != val_in_dev.shape[0]) {
      throw std::invalid_argument("key_in and val_in must have the same length");
    }
    if ((int64_t)n > key_in_dev.shape[0]) {
      throw std::invalid_argument("n exceeds len(key_in)");
    }

    if (key_out_dev.shape.size() != 1 || val_out_dev.shape.size() != 1) {
      throw std::invalid_argument("key_out and val_out must be 1D device arrays");
    }
    if (key_out_dev.shape[0] < (int64_t)n || val_out_dev.shape[0] < (int64_t)n) {
      throw std::invalid_argument("key_out/val_out must have length >= n");
    }

    if (!key_in_dev.strides_bytes.empty()) {
      if (key_in_dev.strides_bytes.size() != 1) throw std::invalid_argument("key_in must be 1D with valid strides");
      if (key_in_dev.strides_bytes[0] != (int64_t)sizeof(uint64_t)) throw std::invalid_argument("key_in must be contiguous");
    }
    ...
    if (!key_out_dev.strides_bytes.empty()) {
      if (key_out_dev.strides_bytes.size() != 1) throw std::invalid_argument("key_out must be 1D with valid strides");
      if (key_out_dev.strides_bytes[0] != (int64_t)sizeof(uint64_t)) throw std::invalid_argument("key_out must be contiguous");
    }
    ...
    throw_on_cuda_error(
        guga_qmc_coalesce_coo_u64_f64_ws_launch_stream(
            ws,
            (const uint64_t*)key_in_dev.ptr,
            (const double*)val_in_dev.ptr,
            n,
            (uint64_t*)key_out_dev.ptr,
    ...
    if (sync) {
      throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_ws_coalesce_u64)");
    }
  }

  void phi_pivot_resample_u64_f64_inplace_device(
      py::object key_in,
      py::object val_in,
      py::object key_out,
      py::object val_out,
    ...
    if ((int64_t)n_in > key_in_dev.shape[0]) {
      throw std::invalid_argument("n_in exceeds len(key_in)");
    }

    if (key_out_dev.shape.size() != 1 || val_out_dev.shape.size() != 1) {
      throw std::invalid_argument("key_out and val_out must be 1D device arrays");
    }
    if (key_out_dev.shape[0] < (int64_t)m || val_out_dev.shape[0] < (int64_t)m) {
      throw std::invalid_argument("key_out/val_out must have length >= m");
    }
    ...
    cudaStream_t stream_t = (cudaStream_t)stream;
    throw_on_cuda_error(
        guga_qmc_phi_pivot_resample_u64_f64_ws_launch_stream(
            ws,
            (const uint64_t*)key_in_dev.ptr,
            (const double*)val_in_dev.ptr,
            n_in,
    ...
    if (sync) {
      throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_ws_phi_u64)");
    }
  }
};
...
struct QmcWorkspace {
  ...
  void coalesce_coo_i32_f64_inplace_device(
    ...
    throw_on_cuda_error(
	  ...
	  void sparse_dot_many_sorted_i32_f64_inplace_device(
	      py::object a_idx,
	      py::object a_val,
	      py::object a_nnz,
	      py::object b_idx,
	      ...
	      bool sync) const {
	    if (!ws) throw std::runtime_error("QmcWorkspace has been released");
	    if (threads <= 0 || threads > 1024 || (threads & 31) != 0) {
	      throw std::invalid_argument("threads must be in 32..1024 and a multiple of 32");
	    }
	    if (a_idx.is_none() || a_val.is_none() || a_nnz.is_none() || b_idx.is_none() || b_val.is_none() || b_nnz.is_none() || out.is_none()) {
	      throw std::invalid_argument("a_idx/a_val/a_nnz/b_idx/b_val/b_nnz/out must be device arrays (cannot be None)");
	    }

	    CudaArrayView a_idx_dev = cuda_array_view_from_object(a_idx, "a_idx");
	    CudaArrayView a_val_dev = cuda_array_view_from_object(a_val, "a_val");
	    CudaArrayView a_nnz_dev = cuda_array_view_from_object(a_nnz, "a_nnz");
	    CudaArrayView b_idx_dev = cuda_array_view_from_object(b_idx, "b_idx");
	    CudaArrayView b_val_dev = cuda_array_view_from_object(b_val, "b_val");
	    CudaArrayView b_nnz_dev = cuda_array_view_from_object(b_nnz, "b_nnz");
	    CudaArrayView out_dev = cuda_array_view_from_object(out, "out");

	    require_typestr(a_idx_dev, "a_idx", "<i4");
	    require_typestr(a_val_dev, "a_val", "<f8");
	    require_typestr(a_nnz_dev, "a_nnz", "<i4");
	    require_typestr(b_idx_dev, "b_idx", "<i4");
	    require_typestr(b_val_dev, "b_val", "<f8");
	    require_typestr(b_nnz_dev, "b_nnz", "<i4");
	    ...
	    int nA = 0;
	    int stride_a = 0;
	    if (a_idx_dev.shape.size() == 1) {
	      nA = 1;
	      stride_a = (int)a_idx_dev.shape[0];
	    } else if (a_idx_dev.shape.size() == 2) {
	      nA = (int)a_idx_dev.shape[0];
	      stride_a = (int)a_idx_dev.shape[1];
	    } else {
	      throw std::invalid_argument("a_idx must be a 1D or 2D device array");
	    }

	    int nB = 0;
	    int stride_b = 0;
	    if (b_idx_dev.shape.size() == 1) {
	      nB = 1;
	      stride_b = (int)b_idx_dev.shape[0];
	    } else if (b_idx_dev.shape.size() == 2) {
	      nB = (int)b_idx_dev.shape[0];
	      stride_b = (int)b_idx_dev.shape[1];
	    } else {
	      throw std::invalid_argument("b_idx must be a 1D or 2D device array");
	    }

	    // Validate shapes match for idx/val pairs.
	    if (a_val_dev.shape != a_idx_dev.shape) {
	      throw std::invalid_argument("a_val must have the same shape as a_idx");
	    }
	    if (b_val_dev.shape != b_idx_dev.shape) {
	      throw std::invalid_argument("b_val must have the same shape as b_idx");
	    }

	    // Row-major contiguous checks.
	    if (!a_idx_dev.strides_bytes.empty()) {
	      if (a_idx_dev.shape.size() == 1) {
	        if (a_idx_dev.strides_bytes.size() != 1 || a_idx_dev.strides_bytes[0] != (int64_t)sizeof(int32_t)) {
	          throw std::invalid_argument("a_idx must be contiguous");
	        }
	      } else {
	        if (a_idx_dev.strides_bytes.size() != 2) throw std::invalid_argument("a_idx strides must have length 2");
	        int64_t s0 = a_idx_dev.strides_bytes[0];
	        int64_t s1 = a_idx_dev.strides_bytes[1];
	        if (s0 != (int64_t)stride_a * (int64_t)sizeof(int32_t) || s1 != (int64_t)sizeof(int32_t)) {
	          throw std::invalid_argument("a_idx must be C-contiguous with no padding");
	        }
	      }
	    }
	    if (!a_val_dev.strides_bytes.empty()) {
	      if (a_val_dev.shape.size() == 1) {
	        if (a_val_dev.strides_bytes.size() != 1 || a_val_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
	          throw std::invalid_argument("a_val must be contiguous");
	        }
	      } else {
	        if (a_val_dev.strides_bytes.size() != 2) throw std::invalid_argument("a_val strides must have length 2");
	        int64_t s0 = a_val_dev.strides_bytes[0];
	        int64_t s1 = a_val_dev.strides_bytes[1];
	        if (s0 != (int64_t)stride_a * (int64_t)sizeof(double) || s1 != (int64_t)sizeof(double)) {
	          throw std::invalid_argument("a_val must be C-contiguous with no padding");
	        }
	      }
	    }
	    if (!b_idx_dev.strides_bytes.empty()) {
	      if (b_idx_dev.shape.size() == 1) {
	        if (b_idx_dev.strides_bytes.size() != 1 || b_idx_dev.strides_bytes[0] != (int64_t)sizeof(int32_t)) {
	          throw std::invalid_argument("b_idx must be contiguous");
	        }
	      } else {
	        if (b_idx_dev.strides_bytes.size() != 2) throw std::invalid_argument("b_idx strides must have length 2");
	        int64_t s0 = b_idx_dev.strides_bytes[0];
	        int64_t s1 = b_idx_dev.strides_bytes[1];
	        if (s0 != (int64_t)stride_b * (int64_t)sizeof(int32_t) || s1 != (int64_t)sizeof(int32_t)) {
	          throw std::invalid_argument("b_idx must be C-contiguous with no padding");
	        }
	      }
	    }
	    if (!b_val_dev.strides_bytes.empty()) {
	      if (b_val_dev.shape.size() == 1) {
	        if (b_val_dev.strides_bytes.size() != 1 || b_val_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
	          throw std::invalid_argument("b_val must be contiguous");
	        }
	      } else {
	        if (b_val_dev.strides_bytes.size() != 2) throw std::invalid_argument("b_val strides must have length 2");
	        int64_t s0 = b_val_dev.strides_bytes[0];
	        int64_t s1 = b_val_dev.strides_bytes[1];
	        if (s0 != (int64_t)stride_b * (int64_t)sizeof(double) || s1 != (int64_t)sizeof(double)) {
	          throw std::invalid_argument("b_val must be C-contiguous with no padding");
	        }
	      }
	    }

	    // nnz arrays: shape (n,) or scalar (only allowed when n==1).
	    if (a_nnz_dev.shape.size() == 1) {
	      if (a_nnz_dev.shape[0] != (int64_t)nA) throw std::invalid_argument("a_nnz must have length nA");
	      if (!a_nnz_dev.strides_bytes.empty()) {
	        if (a_nnz_dev.strides_bytes.size() != 1 || a_nnz_dev.strides_bytes[0] != (int64_t)sizeof(int32_t)) {
	          throw std::invalid_argument("a_nnz must be contiguous");
	        }
	      }
	    } else if (a_nnz_dev.shape.size() == 0) {
	      if (nA != 1) throw std::invalid_argument("a_nnz scalar is only valid for nA==1");
	    } else {
	      throw std::invalid_argument("a_nnz must have shape (nA,) or ()");
	    }

	    if (b_nnz_dev.shape.size() == 1) {
	      if (b_nnz_dev.shape[0] != (int64_t)nB) throw std::invalid_argument("b_nnz must have length nB");
	      if (!b_nnz_dev.strides_bytes.empty()) {
	        if (b_nnz_dev.strides_bytes.size() != 1 || b_nnz_dev.strides_bytes[0] != (int64_t)sizeof(int32_t)) {
	          throw std::invalid_argument("b_nnz must be contiguous");
	        }
	      }
	    } else if (b_nnz_dev.shape.size() == 0) {
	      if (nB != 1) throw std::invalid_argument("b_nnz scalar is only valid for nB==1");
	    } else {
	      throw std::invalid_argument("b_nnz must have shape (nB,) or ()");
	    }

	    // Output: (nA,nB) row-major.
	    if (out_dev.shape.size() != 2 || out_dev.shape[0] != (int64_t)nA || out_dev.shape[1] != (int64_t)nB) {
	      throw std::invalid_argument("out must have shape (nA,nB)");
	    }
	    if (!out_dev.strides_bytes.empty()) {
	      if (out_dev.strides_bytes.size() != 2) throw std::invalid_argument("out strides must have length 2");
	      int64_t s0 = out_dev.strides_bytes[0];
	      int64_t s1 = out_dev.strides_bytes[1];
	      if (s0 != (int64_t)nB * (int64_t)sizeof(double) || s1 != (int64_t)sizeof(double)) {
	        throw std::invalid_argument("out must be C-contiguous with no padding");
	      }
	    }

	    cudaStream_t stream_t = (cudaStream_t)stream;
	    throw_on_cuda_error(
	        guga_qmc_sparse_dot_many_sorted_i32_f64_launch_stream(
	            (const int32_t*)a_idx_dev.ptr,
	            (const double*)a_val_dev.ptr,
	            (const int32_t*)a_nnz_dev.ptr,
	            nA,
	            stride_a,
	            (const int32_t*)b_idx_dev.ptr,
	            (const double*)b_val_dev.ptr,
	            (const int32_t*)b_nnz_dev.ptr,
	            nB,
	            stride_b,
	            (double*)out_dev.ptr,
	            stream_t,
	    ...
	    if (sync) {
	      throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_ws_sparse_dot_many)");
	    }
	  }

	  int fciqmc_step_shift_i32_f64_inplace_device(
	      const DeviceDRT& drt,
	      const DeviceStateCache& state,
	      py::object x_idx,
	      py::object x_val,
	    ...
	    if (threads_spawn <= 0 || threads_spawn > 1024) {
	      throw std::invalid_argument("threads_spawn must be in 1..1024");
	    }
	    if (threads_qmc <= 0 || threads_qmc > 1024) {
	      throw std::invalid_argument("threads_qmc must be in 1..1024");
	    }
	    ...
	    if (nnz < 0 || nnz > max_m) {
	      throw std::invalid_argument("nnz out of range for workspace max_m");
	    }
	    ...
	    if (x_idx.is_none() || x_val.is_none() || h_base_flat.is_none() || eri_mat.is_none() || x_idx_out.is_none() || x_val_out.is_none() ||
	        nnz_out.is_none()) {
	      throw std::invalid_argument("inputs/outputs must be device arrays (cannot be None)");
	    }
	    ...
	    CudaArrayView x_idx_out_dev = cuda_array_view_from_object(x_idx_out, "x_idx_out");
	    CudaArrayView x_val_out_dev = cuda_array_view_from_object(x_val_out, "x_val_out");
	    CudaArrayView nnz_out_dev = cuda_array_view_from_object(nnz_out, "nnz_out");
	    ...
	    require_typestr(x_idx_out_dev, "x_idx_out", "<i4");
	    require_typestr(x_val_out_dev, "x_val_out", "<f8");
	    require_typestr(nnz_out_dev, "nnz_out", "<i4");

	    if (x_idx_out_dev.read_only || x_val_out_dev.read_only || nnz_out_dev.read_only) {
	      throw std::invalid_argument("output buffers must be writable");
	    }

	    if (x_idx_dev.shape.size() != 1 || x_val_dev.shape.size() != 1) {
	      throw std::invalid_argument("x_idx and x_val must be 1D arrays");
	    }
	    if (x_idx_dev.shape[0] < (int64_t)nnz || x_val_dev.shape[0] < (int64_t)nnz) {
	      throw std::invalid_argument("x_idx/x_val length must be >= nnz");
	    }
	    ...
	    int64_t out_len64 = (int64_t)nnz * (int64_t)nspawn_total;
	    if (out_len64 < 0 || out_len64 > (int64_t)std::numeric_limits<int>::max()) {
	      throw std::invalid_argument("event buffer length out of supported range");
	    }
	    int out_len = (int)out_len64;
	    int64_t all_len64 = (int64_t)nnz + (int64_t)out_len;
	    if (all_len64 < 0 || all_len64 > (int64_t)max_n) {
	      throw std::invalid_argument("merged buffer length out of range for workspace max_n");
	    }
	    int all_len = (int)all_len64;

	    if (x_idx_dev.shape[0] < (int64_t)all_len || x_val_dev.shape[0] < (int64_t)all_len) {
	      throw std::invalid_argument("x_idx/x_val buffers must have length >= nnz + nnz*(nspawn_one+nspawn_two)");
	    }
	    if (x_idx_out_dev.shape.size() != 1 || x_val_out_dev.shape.size() != 1) {
	      throw std::invalid_argument("x_idx_out and x_val_out must be 1D arrays");
	    }
	    if (x_idx_out_dev.shape[0] < (int64_t)all_len || x_val_out_dev.shape[0] < (int64_t)all_len) {
	      throw std::invalid_argument("x_idx_out/x_val_out buffers must have length >= merged length");
	    }

	    // Device scalar checks for nnz_out.
	    if (nnz_out_dev.shape.size() == 1) {
	      if (nnz_out_dev.shape[0] != 1) throw std::invalid_argument("nnz_out must have shape (1,) or ()");
	      if (!nnz_out_dev.strides_bytes.empty()) {
	        if (nnz_out_dev.strides_bytes.size() != 1) throw std::invalid_argument("nnz_out must be 1D with valid strides");
	        if (nnz_out_dev.strides_bytes[0] != (int64_t)sizeof(int32_t)) throw std::invalid_argument("nnz_out must be contiguous");
	      }
	    } else if (nnz_out_dev.shape.size() == 0) {
	      // ok
	    } else {
	      throw std::invalid_argument("nnz_out must have shape (1,) or ()");
	    }

	    cudaStream_t stream_t = (cudaStream_t)stream;

	    // Event buffer is the tail of the merged input.
	    int32_t* d_x_idx = (int32_t*)x_idx_dev.ptr;
	    double* d_x_val = (double*)x_val_dev.ptr;
	    int32_t* d_evt_idx = d_x_idx + nnz;
	    double* d_evt_val = d_x_val + nnz;
	    throw_on_cuda_error(cudaMemsetAsync(d_evt_idx, 0xFF, (size_t)out_len * sizeof(int32_t), stream_t), "cudaMemsetAsync(evt_idx=-1)");
	    throw_on_cuda_error(cudaMemsetAsync(d_evt_val, 0, (size_t)out_len * sizeof(double), stream_t), "cudaMemsetAsync(evt_val=0)");

	    // Spawn into tail (uses original x_val prefix).
	    throw_on_cuda_error(
	    ...
	    // Scale identity term in-place in the prefix: x <- (1 + dt*shift) * x.
	    double scale = 1.0 + dt * shift;
	    throw_on_cuda_error(
	        guga_qmc_pack_scaled_identity_i32_f64_launch_stream(
	            (const int32_t*)x_idx_dev.ptr,
	            (const double*)x_val_dev.ptr,
	            nnz,
	            scale,
	    ...
	    throw_on_cuda_error(
	        guga_qmc_coalesce_coo_i32_f64_ws_launch_stream(
	            ...
	            d_x_val,
	            all_len,
	            (int32_t*)x_idx_out_dev.ptr,
	            (double*)x_val_out_dev.ptr,
	            (int*)nnz_out_dev.ptr,
	            stream_t,
	            threads_qmc),
	        "guga_qmc_coalesce_coo_i32_f64_ws_launch_stream");

	    // Read final nnz (required since we return it).
	    int n_out = 0;
	    throw_on_cuda_error(cudaMemcpyAsync(&n_out, nnz_out_dev.ptr, sizeof(int), cudaMemcpyDeviceToHost, stream_t), "cudaMemcpyAsync(nnz_out)");
	    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(read nnz_out)");
	    if (n_out < 0) n_out = 0;
	    if (n_out > max_m) {
	      throw std::runtime_error("walker population after coalescing exceeds max_m for workspace");
	    }

	    (void)sync;  // Return value requires synchronization regardless of this flag.
	    return n_out;
	  }
	};
...
void epq_apply_gather_inplace_device(
  ...
  if (!x_dev.strides_bytes.empty()) {
    ...
    int64_t s0 = x_dev.strides_bytes[0];
    if (s0 <= 0) throw std::invalid_argument("x must have positive row stride");
    if (s0 % (int64_t)sizeof(double) != 0) {
      throw std::invalid_argument("x row stride must be a multiple of itemsize");
    }
    if (s0 < (int64_t)nvec * (int64_t)sizeof(double)) {
      throw std::invalid_argument("x row stride too small");
    }
  }
  if (!y_dev.strides_bytes.empty()) {
    ...
    int64_t s0 = y_dev.strides_bytes[0];
    if (s0 <= 0) throw std::invalid_argument("y must have positive row stride");
    if (s0 % (int64_t)sizeof(double) != 0) {
      throw std::invalid_argument("y row stride must be a multiple of itemsize");
    }
    if (s0 < (int64_t)nvec * (int64_t)sizeof(double)) {
      throw std::invalid_argument("y row stride too small");
    }
  }
  ...
  if (!x_dev.strides_bytes.empty()) {
    int64_t s0 = x_dev.strides_bytes[0];
    ldx = (int)(s0 / (int64_t)sizeof(double));
  }
  int ldy = nvec;
  if (!y_dev.strides_bytes.empty()) {
    int64_t s0 = y_dev.strides_bytes[0];
    ldy = (int)(s0 / (int64_t)sizeof(double));
  }
...
void apply_g_flat_scatter_atomic_frontier_hash_inplace_device(
    const DeviceDRT& drt,
    const DeviceStateCache& state,
    py::object task_csf,
    py::object task_g,
  ...
  if (hash_keys.is_none() || hash_vals.is_none()) {
    throw std::invalid_argument("hash_keys and hash_vals must be device arrays (cannot be None)");
  }
  if (overflow.is_none()) {
    throw std::invalid_argument("overflow must be a device array of shape (1,) (cannot be None)");
  }
  ...
  int64_t fp_itemsize = (int64_t)sizeof(double);
  ...
  if (!task_scale.is_none()) {
    ...
    d_task_scale_f64 = reinterpret_cast<const double*>(task_scale_dev.ptr);
  }

  auto hash_keys_dev = cuda_array_view_from_object(hash_keys, "hash_keys");
  require_typestr(hash_keys_dev, "hash_keys", "<i4");
  if (hash_keys_dev.read_only) {
    throw std::invalid_argument("hash_keys must be writable");
  }
  if (hash_keys_dev.shape.size() != 1) {
    throw std::invalid_argument("hash_keys must be 1D device array");
  }
  if (!hash_keys_dev.strides_bytes.empty()) {
    if (hash_keys_dev.strides_bytes.size() != 1 || hash_keys_dev.strides_bytes[0] != (int64_t)sizeof(int32_t)) {
      throw std::invalid_argument("hash_keys must be contiguous");
    }
  }
  int64_t cap_ll = hash_keys_dev.shape[0];
  if (cap_ll <= 0 || cap_ll > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("hash_keys capacity out of supported range");
  }
  int cap = (int)cap_ll;
  if ((cap & (cap - 1)) != 0) {
    throw std::invalid_argument("hash_keys capacity must be a power of two");
  }

  auto hash_vals_dev = cuda_array_view_from_object(hash_vals, "hash_vals");
  require_typestr(hash_vals_dev, "hash_vals", "<f8");
  if (hash_vals_dev.read_only) {
    throw std::invalid_argument("hash_vals must be writable");
  }
  if (hash_vals_dev.shape.size() != 2) {
    throw std::invalid_argument("hash_vals must be a 2D device array with shape (nroots, cap)");
  }
  int nroots = (int)hash_vals_dev.shape[0];
  int cap2 = (int)hash_vals_dev.shape[1];
  if (cap2 != cap) {
    throw std::invalid_argument("hash_vals must have shape (nroots, cap) with cap matching hash_keys");
  }
  if (root < 0 || root >= nroots) {
    throw std::invalid_argument("root out of range for hash_vals");
  }
  if (!hash_vals_dev.strides_bytes.empty()) {
    if (hash_vals_dev.strides_bytes.size() != 2) {
      throw std::invalid_argument("hash_vals strides must have length 2");
    }
    int64_t s0 = hash_vals_dev.strides_bytes[0];
    int64_t s1 = hash_vals_dev.strides_bytes[1];
    if (s0 <= 0 || s1 <= 0) {
      throw std::invalid_argument("hash_vals must have positive strides");
    }
    if (s1 != (int64_t)sizeof(double)) {
      throw std::invalid_argument("hash_vals must be C-contiguous along last dimension");
    }
    if (s0 != (int64_t)cap * (int64_t)sizeof(double)) {
      throw std::invalid_argument("hash_vals must be C-contiguous with no padding");
    }
  }

  const uint8_t* d_selected_mask = nullptr;
  uint64_t selected_stream = 0;
  if (!selected_mask.is_none()) {
    auto sel_dev = cuda_array_view_from_object(selected_mask, "selected_mask");
    require_typestr(sel_dev, "selected_mask", "|u1");
    if (sel_dev.shape.size() != 1 || sel_dev.shape[0] != (int64_t)state.ncsf) {
      throw std::invalid_argument("selected_mask must have shape (ncsf,)");
    }
    if (!sel_dev.strides_bytes.empty()) {
      if (sel_dev.strides_bytes.size() != 1 || sel_dev.strides_bytes[0] != (int64_t)sizeof(uint8_t)) {
        throw std::invalid_argument("selected_mask must be contiguous");
      }
    }
    d_selected_mask = reinterpret_cast<const uint8_t*>(sel_dev.ptr);
    selected_stream = sel_dev.stream;
  }
  ...
  const int32_t* d_task_csf = reinterpret_cast<const int32_t*>(task_csf_dev.ptr);
  const double* d_task_g_f64 = reinterpret_cast<const double*>(task_g_dev.ptr);
  int32_t* d_hash_keys = reinterpret_cast<int32_t*>(hash_keys_dev.ptr);
  double* d_hash_vals = reinterpret_cast<double*>(hash_vals_dev.ptr);
  int* d_overflow = reinterpret_cast<int*>(overflow_dev.ptr);

  // NOTE: overflow is treated as a sticky flag across multiple kernel launches
  // (e.g., when building a frontier hash map in stages). Caller is responsible
  // for clearing it before a sequence of launches.

  if (ntasks > 0) {
    guga_apply_g_flat_scatter_atomic_frontier_hash_launch_stream(
        drt.child,
        drt.node_twos,
        drt.child_prefix,
        state.steps,
        ...
        stream_t,
        threads);
    throw_on_cuda_error(cudaGetLastError(), "kernel launch(apply_g_flat_scatter_atomic_frontier_hash)");
  }

  int h_overflow = 0;
  if (check_overflow) {
    throw_on_cuda_error(cudaMemcpyAsync(&h_overflow, d_overflow, sizeof(int), cudaMemcpyDeviceToHost, stream_t),
                        "D2H overflow(frontier_hash)");
  }
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(apply_g_flat_scatter_atomic_frontier_hash)");
  }
  if (check_overflow && h_overflow) {
    throw std::runtime_error("apply_g_flat_scatter_atomic_frontier_hash overflow (hash full or segment-walk overflow)");
  }
}

void apply_g_flat_scatter_atomic_frontier_hash_many_roots_inplace_device(
    const DeviceDRT& drt,
    const DeviceStateCache& state,
    py::object task_csf,
    py::object task_g,
    py::object task_scale_task_major,
    py::object hash_keys,
    py::object hash_vals,
    py::object selected_mask,
    py::object overflow,
  ...
  if (task_scale_task_major.is_none()) {
    throw std::invalid_argument("task_scale_task_major must be provided");
  }
  if (hash_keys.is_none() || hash_vals.is_none()) {
    throw std::invalid_argument("hash_keys and hash_vals must be device arrays (cannot be None)");
  }
  ...
  auto scale_dev = cuda_array_view_from_object(task_scale_task_major, "task_scale_task_major");
  require_typestr(scale_dev, "task_scale_task_major", "<f8");
  uint64_t scale_stream = scale_dev.stream;
  if (scale_dev.shape.size() != 2 || (scale_dev.shape[0] != ntasks_ll && scale_dev.shape[0] != 1)) {
    throw std::invalid_argument("task_scale_task_major must have shape (ntasks, nroots) or (1, nroots)");
  }
  if (!scale_dev.strides_bytes.empty()) {
    if (scale_dev.strides_bytes.size() != 2) {
      throw std::invalid_argument("task_scale_task_major strides must have length 2");
    }
    int64_t s0 = scale_dev.strides_bytes[0];
    int64_t s1 = scale_dev.strides_bytes[1];
    if (s0 <= 0 || s1 <= 0) {
      throw std::invalid_argument("task_scale_task_major must have positive strides");
    }
    if (s1 != fp_itemsize) {
      throw std::invalid_argument("task_scale_task_major must be C-contiguous along last dimension");
    }
    if (s0 != (int64_t)scale_dev.shape[1] * fp_itemsize) {
      throw std::invalid_argument("task_scale_task_major must be C-contiguous with no padding");
    }
  }
  int64_t nroots_ll = scale_dev.shape[1];
  if (nroots_ll <= 0 || nroots_ll > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("task_scale_task_major second dimension out of supported range");
  }
  int nroots = (int)nroots_ll;
  int64_t task_scale_stride = (scale_dev.shape[0] == 1) ? 0 : (int64_t)nroots;
  ...
  if (hash_vals_dev.shape[0] != (int64_t)nroots || hash_vals_dev.shape[1] != (int64_t)cap) {
    throw std::invalid_argument("hash_vals must have shape (nroots, cap) matching task_scale_task_major and hash_keys");
  }
  if (!hash_vals_dev.strides_bytes.empty()) {
    if (hash_vals_dev.strides_bytes.size() != 2) {
      throw std::invalid_argument("hash_vals strides must have length 2");
  ...
  if (stream_u == 0) {
    ...
    else if (scale_stream) stream_u = scale_stream;
    else if (selected_stream) stream_u = selected_stream;
    else if (overflow_dev.stream) stream_u = overflow_dev.stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);
  ...
  const double* d_task_scale_task_major = reinterpret_cast<const double*>(scale_dev.ptr);
  int32_t* d_hash_keys = reinterpret_cast<int32_t*>(hash_keys_dev.ptr);
  double* d_hash_vals = reinterpret_cast<double*>(hash_vals_dev.ptr);
  int* d_overflow = reinterpret_cast<int*>(overflow_dev.ptr);

  if (ntasks > 0) {
    guga_apply_g_flat_scatter_atomic_frontier_hash_many_roots_launch_stream(
        drt.child,
        drt.node_twos,
        drt.child_prefix,
        state.steps,
        ...
        stream_t,
        threads);
    throw_on_cuda_error(cudaGetLastError(), "kernel launch(apply_g_flat_scatter_atomic_frontier_hash_many_roots)");
  }

  int h_overflow = 0;
  if (check_overflow) {
    throw_on_cuda_error(cudaMemcpyAsync(&h_overflow, d_overflow, sizeof(int), cudaMemcpyDeviceToHost, stream_t),
                        "D2H overflow(frontier_hash_many_roots)");
  }
  if (sync) {
    throw_on_cuda_error(
        cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(apply_g_flat_scatter_atomic_frontier_hash_many_roots)");
  }
  if (check_overflow && h_overflow) {
    throw std::runtime_error("apply_g_flat_scatter_atomic_frontier_hash_many_roots overflow (hash full or segment-walk overflow)");
  }
}

void cipsi_frontier_hash_clear_inplace_device(
    py::object keys,
    py::object vals_root_major,
    int threads,
    uint64_t stream,
    bool sync) {
  if (threads <= 0 || threads > 1024) {
  ...
  if (keys.is_none() || vals_root_major.is_none()) {
    throw std::invalid_argument("keys and vals_root_major must be device arrays (cannot be None)");
  }
  auto keys_dev = cuda_array_view_from_object(keys, "keys");
  auto vals_dev = cuda_array_view_from_object(vals_root_major, "vals_root_major");
  require_typestr(keys_dev, "keys", "<i4");
  require_typestr(vals_dev, "vals_root_major", "<f8");
  if (keys_dev.read_only || vals_dev.read_only) {
    throw std::invalid_argument("keys and vals_root_major must be writable");
  }
  if (keys_dev.shape.size() != 1) {
    throw std::invalid_argument("keys must be 1D device array");
  }
  if (vals_dev.shape.size() != 2) {
    throw std::invalid_argument("vals_root_major must be 2D device array with shape (nroots, cap)");
  }
  int64_t cap_ll = keys_dev.shape[0];
  if (cap_ll <= 0 || cap_ll > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("cap out of supported range");
  }
  int cap = (int)cap_ll;
  int nroots = (int)vals_dev.shape[0];
  int cap2 = (int)vals_dev.shape[1];
  if (cap2 != cap) {
    throw std::invalid_argument("vals_root_major must have shape (nroots, cap) with cap matching keys length");
  }
  if ((cap & (cap - 1)) != 0) {
    throw std::invalid_argument("cap must be a power of two");
  }
  if (!keys_dev.strides_bytes.empty() &&
      (keys_dev.strides_bytes.size() != 1 || keys_dev.strides_bytes[0] != (int64_t)sizeof(int32_t))) {
    throw std::invalid_argument("keys must be contiguous");
  }
  if (!vals_dev.strides_bytes.empty()) {
    if (vals_dev.strides_bytes.size() != 2) {
      throw std::invalid_argument("vals_root_major strides must have length 2");
    }
    int64_t s0 = vals_dev.strides_bytes[0];
    int64_t s1 = vals_dev.strides_bytes[1];
    if (s0 <= 0 || s1 <= 0) {
      throw std::invalid_argument("vals_root_major must have positive strides");
    }
    if (s1 != (int64_t)sizeof(double)) {
      throw std::invalid_argument("vals_root_major must be C-contiguous along last dimension");
    }
    if (s0 != (int64_t)cap * (int64_t)sizeof(double)) {
      throw std::invalid_argument("vals_root_major must be C-contiguous with no padding");
    }
  }

  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (vals_dev.stream) stream_u = vals_dev.stream;
    else if (keys_dev.stream) stream_u = keys_dev.stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);

  int32_t* d_keys = reinterpret_cast<int32_t*>(keys_dev.ptr);
  double* d_vals = reinterpret_cast<double*>(vals_dev.ptr);
  throw_on_cuda_error(
      guga_cipsi_frontier_hash_clear_launch_stream(d_keys, d_vals, cap, nroots, stream_t, threads),
      "guga_cipsi_frontier_hash_clear_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cipsi_frontier_hash_clear)");
  }
}

void cipsi_frontier_hash_extract_inplace_device(
    py::object keys,
    py::object vals_root_major,
    py::object out_idx,
    py::object out_vals_root_major,
    py::object out_nnz,
    int threads,
    uint64_t stream,
    bool sync) {
  ...
  if (keys.is_none() || vals_root_major.is_none() || out_idx.is_none() || out_vals_root_major.is_none() || out_nnz.is_none()) {
    throw std::invalid_argument("inputs/outputs must be device arrays (cannot be None)");
  }
  auto keys_dev = cuda_array_view_from_object(keys, "keys");
  auto vals_dev = cuda_array_view_from_object(vals_root_major, "vals_root_major");
  auto out_idx_dev = cuda_array_view_from_object(out_idx, "out_idx");
  auto out_vals_dev = cuda_array_view_from_object(out_vals_root_major, "out_vals_root_major");
  auto out_nnz_dev = cuda_array_view_from_object(out_nnz, "out_nnz");
  require_typestr(keys_dev, "keys", "<i4");
  require_typestr(vals_dev, "vals_root_major", "<f8");
  require_typestr(out_idx_dev, "out_idx", "<i4");
  require_typestr(out_vals_dev, "out_vals_root_major", "<f8");
  require_typestr(out_nnz_dev, "out_nnz", "<i4");
  if (keys_dev.read_only || vals_dev.read_only || out_idx_dev.read_only || out_vals_dev.read_only || out_nnz_dev.read_only) {
    throw std::invalid_argument("output arrays must be writable");
  }
  if (keys_dev.shape.size() != 1) {
    throw std::invalid_argument("keys must be 1D");
  }
  if (vals_dev.shape.size() != 2) {
    throw std::invalid_argument("vals_root_major must be 2D with shape (nroots, cap)");
  }
  if (out_idx_dev.shape.size() != 1) {
    throw std::invalid_argument("out_idx must be 1D");
  }
  if (out_vals_dev.shape.size() != 2) {
    throw std::invalid_argument("out_vals_root_major must be 2D with shape (nroots, cap)");
  }
  if (out_nnz_dev.shape.size() != 1 || out_nnz_dev.shape[0] != 1) {
    throw std::invalid_argument("out_nnz must have shape (1,)");
  }
  int64_t cap_ll = keys_dev.shape[0];
  if (cap_ll <= 0 || cap_ll > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("cap out of supported range");
  ...
  int cap2 = (int)vals_dev.shape[1];
  if (cap2 != cap) {
    throw std::invalid_argument("vals_root_major must have shape (nroots, cap) with cap matching keys");
  }
  if ((int)out_idx_dev.shape[0] != cap) {
    throw std::invalid_argument("out_idx must have shape (cap,)");
  }
  if ((int)out_vals_dev.shape[0] != nroots || (int)out_vals_dev.shape[1] != cap) {
    throw std::invalid_argument("out_vals_root_major must have shape (nroots, cap)");
  }
  if ((cap & (cap - 1)) != 0) {
    throw std::invalid_argument("cap must be a power of two");
  }
  ...
  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (out_vals_dev.stream) stream_u = out_vals_dev.stream;
    else if (out_idx_dev.stream) stream_u = out_idx_dev.stream;
    else if (vals_dev.stream) stream_u = vals_dev.stream;
    else if (keys_dev.stream) stream_u = keys_dev.stream;
    else if (out_nnz_dev.stream) stream_u = out_nnz_dev.stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);

  const int32_t* d_keys = reinterpret_cast<const int32_t*>(keys_dev.ptr);
  const double* d_vals = reinterpret_cast<const double*>(vals_dev.ptr);
  int32_t* d_out_idx = reinterpret_cast<int32_t*>(out_idx_dev.ptr);
  double* d_out_vals = reinterpret_cast<double*>(out_vals_dev.ptr);
  int* d_out_nnz = reinterpret_cast<int*>(out_nnz_dev.ptr);

  throw_on_cuda_error(
      guga_cipsi_frontier_hash_extract_launch_stream(d_keys, d_vals, cap, nroots, d_out_idx, d_out_vals, d_out_nnz, stream_t, threads),
      "guga_cipsi_frontier_hash_extract_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cipsi_frontier_hash_extract)");
  }
}

void cipsi_score_and_select_topk_inplace_device(
    py::object idx,
    py::object vals_root_major,
    int nnz,
    py::object e_var,
  ...
  if (idx.is_none() || vals_root_major.is_none() || e_var.is_none() || hdiag.is_none() || out_new_idx.is_none() ||
      out_new_n.is_none() || out_pt2.is_none()) {
    throw std::invalid_argument("idx/vals/e_var/hdiag/out_* must be device arrays (cannot be None)");
  }

  auto idx_dev = cuda_array_view_from_object(idx, "idx");
  auto vals_dev = cuda_array_view_from_object(vals_root_major, "vals_root_major");
  auto e_var_dev = cuda_array_view_from_object(e_var, "e_var");
  auto hdiag_dev = cuda_array_view_from_object(hdiag, "hdiag");
  auto out_new_idx_dev = cuda_array_view_from_object(out_new_idx, "out_new_idx");
  auto out_new_n_dev = cuda_array_view_from_object(out_new_n, "out_new_n");
  auto out_pt2_dev = cuda_array_view_from_object(out_pt2, "out_pt2");

  require_typestr(idx_dev, "idx", "<i4");
  require_typestr(vals_dev, "vals_root_major", "<f8");
  require_typestr(e_var_dev, "e_var", "<f8");
  require_typestr(hdiag_dev, "hdiag", "<f8");
  require_typestr(out_new_idx_dev, "out_new_idx", "<i4");
  require_typestr(out_new_n_dev, "out_new_n", "<i4");
  require_typestr(out_pt2_dev, "out_pt2", "<f8");

  if (idx_dev.shape.size() != 1) throw std::invalid_argument("idx must be 1D");
  if (vals_dev.shape.size() != 2) throw std::invalid_argument("vals_root_major must be 2D with shape (nroots, stride)");
  if (e_var_dev.shape.size() != 1) throw std::invalid_argument("e_var must be 1D");
  if (hdiag_dev.shape.size() != 1) throw std::invalid_argument("hdiag must be 1D");
  if (out_new_idx_dev.shape.size() != 1) throw std::invalid_argument("out_new_idx must be 1D");
  if (out_new_n_dev.shape.size() != 1 || out_new_n_dev.shape[0] != 1) throw std::invalid_argument("out_new_n must have shape (1,)");
  if (out_pt2_dev.shape.size() != 1) throw std::invalid_argument("out_pt2 must be 1D");

  int64_t nnz_ll = idx_dev.shape[0];
  if (nnz_ll < 0 || nnz_ll > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("idx too large");
  }
  int nnz_max = (int)nnz_ll;
  if (nnz < 0 || nnz > nnz_max) {
    throw std::invalid_argument("nnz out of range for idx array");
  }

  int nroots = (int)vals_dev.shape[0];
  int64_t vals_stride = (int64_t)vals_dev.shape[1];
  if ((int)e_var_dev.shape[0] != nroots) {
    throw std::invalid_argument("e_var must have shape (nroots,)");
  }
  if ((int)out_pt2_dev.shape[0] != nroots) {
    throw std::invalid_argument("out_pt2 must have shape (nroots,)");
  }
  if (vals_stride < (int64_t)nnz) {
    throw std::invalid_argument("vals_root_major second dimension (stride) must be >= nnz");
  }

  int64_t ncsf_ll = hdiag_dev.shape[0];
  if (ncsf_ll < 0 || ncsf_ll > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("hdiag too large");
  }
  int ncsf = (int)ncsf_ll;
  ...
  if (!selected_mask.is_none()) {
    auto sel_dev = cuda_array_view_from_object(selected_mask, "selected_mask");
    require_typestr(sel_dev, "selected_mask", "|u1");
    if (sel_dev.shape.size() != 1 || sel_dev.shape[0] != (int64_t)ncsf) {
      throw std::invalid_argument("selected_mask must have shape (ncsf,)");
    }
    if (!sel_dev.strides_bytes.empty()) {
      if (sel_dev.strides_bytes.size() != 1 || sel_dev.strides_bytes[0] != (int64_t)sizeof(uint8_t)) {
  ...
  int max_add = (int)out_new_idx_dev.shape[0];
  if (max_add < 0) max_add = 0;

  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (out_pt2_dev.stream) stream_u = out_pt2_dev.stream;
    else if (out_new_n_dev.stream) stream_u = out_new_n_dev.stream;
    else if (out_new_idx_dev.stream) stream_u = out_new_idx_dev.stream;
    else if (vals_dev.stream) stream_u = vals_dev.stream;
    else if (idx_dev.stream) stream_u = idx_dev.stream;
    else if (e_var_dev.stream) stream_u = e_var_dev.stream;
    else if (hdiag_dev.stream) stream_u = hdiag_dev.stream;
    else if (selected_stream) stream_u = selected_stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);

  const int32_t* d_idx = reinterpret_cast<const int32_t*>(idx_dev.ptr);
  const double* d_vals = reinterpret_cast<const double*>(vals_dev.ptr);
  const double* d_e_var = reinterpret_cast<const double*>(e_var_dev.ptr);
  const double* d_hdiag = reinterpret_cast<const double*>(hdiag_dev.ptr);
  int32_t* d_out_new_idx = reinterpret_cast<int32_t*>(out_new_idx_dev.ptr);
  int* d_out_new_n = reinterpret_cast<int*>(out_new_n_dev.ptr);
  double* d_out_pt2 = reinterpret_cast<double*>(out_pt2_dev.ptr);

  throw_on_cuda_error(
      guga_cipsi_score_and_select_topk_launch_stream(
          d_idx,
          d_vals,
          vals_stride,
          int(nnz),
      ...
      "guga_cipsi_score_and_select_topk_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cipsi_score_and_select_topk)");
  }
}

void cipsi_score_and_select_topk_from_hash_slots_inplace_device(
    py::object keys_obj,
    py::object vals_root_major_obj,
    py::object e_var_obj,
    py::object hdiag_obj,
    py::object selected_mask_obj,
    double denom_floor,
    py::object out_new_idx_obj,
    py::object out_new_n_obj,
    py::object out_pt2_obj,
    int threads,
    uint64_t stream,
    bool sync) {
  if (threads <= 0 || threads > 1024) {
  ...
  if (keys_obj.is_none() || vals_root_major_obj.is_none() || e_var_obj.is_none() || hdiag_obj.is_none() ||
      out_new_idx_obj.is_none() || out_new_n_obj.is_none() || out_pt2_obj.is_none()) {
    throw std::invalid_argument("inputs/outputs must be device arrays (cannot be None)");
  }

  auto keys_dev = cuda_array_view_from_object(keys_obj, "keys");
  auto vals_dev = cuda_array_view_from_object(vals_root_major_obj, "vals_root_major");
  auto e_var_dev = cuda_array_view_from_object(e_var_obj, "e_var");
  auto hdiag_dev = cuda_array_view_from_object(hdiag_obj, "hdiag");
  auto out_new_idx_dev = cuda_array_view_from_object(out_new_idx_obj, "out_new_idx");
  auto out_new_n_dev = cuda_array_view_from_object(out_new_n_obj, "out_new_n");
  auto out_pt2_dev = cuda_array_view_from_object(out_pt2_obj, "out_pt2");
  ...
  if (vals_dev.shape.size() != 2) {
    throw std::invalid_argument("vals_root_major must have shape (nroots, cap)");
  }
  if (e_var_dev.shape.size() != 1 || hdiag_dev.shape.size() != 1) {
    throw std::invalid_argument("e_var and hdiag must be 1D");
  }
  if (out_new_idx_dev.shape.size() != 1) {
    throw std::invalid_argument("out_new_idx must be 1D");
  }
  if (out_new_n_dev.shape.size() != 1 || out_new_n_dev.shape[0] != 1) {
    throw std::invalid_argument("out_new_n must have shape (1,)");
  }
  if (out_pt2_dev.shape.size() != 1) {
    throw std::invalid_argument("out_pt2 must be 1D");
  }

  int64_t cap_ll = keys_dev.shape[0];
  if (cap_ll <= 0 || cap_ll > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("keys capacity out of supported range");
  }
  int cap = (int)cap_ll;
  int nroots = (int)vals_dev.shape[0];
  int cap2 = (int)vals_dev.shape[1];
  ...
  int ncsf = (int)hdiag_dev.shape[0];
  if ((int)out_pt2_dev.shape[0] != nroots) {
    throw std::invalid_argument("out_pt2 must have shape (nroots,)");
  }
  int max_add = (int)out_new_idx_dev.shape[0];
  ...
  if (!e_var_dev.strides_bytes.empty() &&
      (e_var_dev.strides_bytes.size() != 1 || e_var_dev.strides_bytes[0] != (int64_t)sizeof(double))) {
    throw std::invalid_argument("e_var must be contiguous");
  }
  if (!hdiag_dev.strides_bytes.empty() &&
      (hdiag_dev.strides_bytes.size() != 1 || hdiag_dev.strides_bytes[0] != (int64_t)sizeof(double))) {
    throw std::invalid_argument("hdiag must be contiguous");
  }
  if (!out_new_idx_dev.strides_bytes.empty() &&
      (out_new_idx_dev.strides_bytes.size() != 1 || out_new_idx_dev.strides_bytes[0] != (int64_t)sizeof(int32_t))) {
    throw std::invalid_argument("out_new_idx must be contiguous");
  }
  if (!out_new_n_dev.strides_bytes.empty() &&
      (out_new_n_dev.strides_bytes.size() != 1 || out_new_n_dev.strides_bytes[0] != (int64_t)sizeof(int32_t))) {
    throw std::invalid_argument("out_new_n must be contiguous");
  }
  if (!out_pt2_dev.strides_bytes.empty() &&
      (out_pt2_dev.strides_bytes.size() != 1 || out_pt2_dev.strides_bytes[0] != (int64_t)sizeof(double))) {
    throw std::invalid_argument("out_pt2 must be contiguous");
  }

  const uint8_t* d_selected_mask = nullptr;
  uint64_t selected_stream = 0;
  if (!selected_mask_obj.is_none()) {
    auto sel_dev = cuda_array_view_from_object(selected_mask_obj, "selected_mask");
    require_typestr(sel_dev, "selected_mask", "|u1");
    if (sel_dev.shape.size() != 1 || sel_dev.shape[0] != (int64_t)ncsf) {
      throw std::invalid_argument("selected_mask must have shape (ncsf,)");
    }
  ...
  if (sync) {
    throw_on_cuda_error(
        guga_cipsi_score_and_select_topk_from_hash_slots_launch_stream(
            d_keys,
            d_vals,
            cap,
            nroots,
        ...
        "guga_cipsi_score_and_select_topk_from_hash_slots_launch_stream");
    throw_on_cuda_error(
        cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cipsi_score_and_select_topk_from_hash_slots)");
  } else {
    // Async fallback: avoid any host-side synchronization in the direct hash-slot API.
    throw_on_cuda_error(
        guga_cipsi_score_and_select_topk_launch_stream(
            d_keys,
            d_vals,
            ...
            stream_t,
            int(threads)),
        "guga_cipsi_score_and_select_topk_launch_stream(async_from_hash_slots)");
  }
}

void cipsi_score_and_select_topk_from_hash_slots_v2_inplace_device(
    py::object keys_obj,
    py::object vals_root_major_obj,
    py::object e_var_obj,
    py::object hdiag_obj,
  ...
  if (keys_dev.shape.size() != 1) throw std::invalid_argument("keys must be 1D");
  if (vals_dev.shape.size() != 2) throw std::invalid_argument("vals_root_major must have shape (nroots, cap)");
  if (e_var_dev.shape.size() != 1 || hdiag_dev.shape.size() != 1) throw std::invalid_argument("e_var and hdiag must be 1D");
  if (out_new_idx_dev.shape.size() != 1) throw std::invalid_argument("out_new_idx must be 1D");
  if (out_new_n_dev.shape.size() != 1 || out_new_n_dev.shape[0] != 1) throw std::invalid_argument("out_new_n must have shape (1,)");
  if (out_pt2_dev.shape.size() != 1) throw std::invalid_argument("out_pt2 must be 1D");
  ...
  throw_on_cuda_error(
      guga_cipsi_score_and_select_topk_from_hash_slots_v2_launch_stream(
          d_keys,
          d_vals,
          cap,
          nroots,
  ...
  if (sync) {
    throw_on_cuda_error(
        cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cipsi_score_and_select_topk_from_hash_slots_v2)");
  }
}

void cas36_diag_guess_candidates_u64_dense_inplace_device(
    const DeviceDRT& drt,
    uint64_t ncsf_u64,
    py::object cand_idx_u64_obj,
    py::object h1_diag_obj,
    py::object eri_ppqq_obj,
    py::object eri_pqqp_obj,
    int neleca,
    int nelecb,
    py::object hdiag_out_obj,
    int threads,
    uint64_t stream,
    bool sync) {
  if (drt.child == nullptr || drt.child_prefix == nullptr) {
    throw std::runtime_error("DeviceDRT is not initialized");
  }
  if (drt.norb <= 0 || drt.norb > 64) {
    throw std::invalid_argument("idx64 compact diagonal kernel requires norb in 1..64");
  }
  if (ncsf_u64 == 0ull) {
    throw std::invalid_argument("ncsf must be > 0");
  }
  ...
  if (cand_idx_u64_obj.is_none() || h1_diag_obj.is_none() || eri_ppqq_obj.is_none() || eri_pqqp_obj.is_none() ||
      hdiag_out_obj.is_none()) {
    throw std::invalid_argument("cand_idx_u64/h1_diag/eri_ppqq/eri_pqqp/hdiag_out must be device arrays");
  }

  auto cand_dev = cuda_array_view_from_object(cand_idx_u64_obj, "cand_idx_u64");
  auto h1_diag_dev = cuda_array_view_from_object(h1_diag_obj, "h1_diag");
  auto eri_ppqq_dev = cuda_array_view_from_object(eri_ppqq_obj, "eri_ppqq");
  auto eri_pqqp_dev = cuda_array_view_from_object(eri_pqqp_obj, "eri_pqqp");
  auto hdiag_out_dev = cuda_array_view_from_object(hdiag_out_obj, "hdiag_out");

  require_typestr(cand_dev, "cand_idx_u64", "<u8");
  require_typestr(h1_diag_dev, "h1_diag", "<f8");
  require_typestr(eri_ppqq_dev, "eri_ppqq", "<f8");
  require_typestr(eri_pqqp_dev, "eri_pqqp", "<f8");
  require_typestr(hdiag_out_dev, "hdiag_out", "<f8");

  if (hdiag_out_dev.read_only) {
    throw std::invalid_argument("hdiag_out must be writable");
  }
  if (cand_dev.shape.size() != 1 || h1_diag_dev.shape.size() != 1 || eri_ppqq_dev.shape.size() != 1 ||
      eri_pqqp_dev.shape.size() != 1 || hdiag_out_dev.shape.size() != 1) {
    throw std::invalid_argument("cand_idx_u64/h1_diag/eri_ppqq/eri_pqqp/hdiag_out must be 1D");
  }
  int norb = drt.norb;
  int64_t norb64 = (int64_t)norb;
  int64_t nops64 = norb64 * norb64;
  if (h1_diag_dev.shape[0] != norb64) {
    throw std::invalid_argument("h1_diag must have shape (norb,)");
  }
  if (eri_ppqq_dev.shape[0] != nops64 || eri_pqqp_dev.shape[0] != nops64) {
    throw std::invalid_argument("eri_ppqq/eri_pqqp must have shape (norb*norb,)");
  }
  if (hdiag_out_dev.shape[0] != cand_dev.shape[0]) {
    throw std::invalid_argument("hdiag_out must have same length as cand_idx_u64");
  }

  int64_t ncand64 = cand_dev.shape[0];
  if (ncand64 < 0 || ncand64 > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("candidate length out of range");
  }
  int ncand = (int)ncand64;
  if (ncand == 0) return;

  if (!cand_dev.strides_bytes.empty()) {
    if (cand_dev.strides_bytes.size() != 1 || cand_dev.strides_bytes[0] != (int64_t)sizeof(uint64_t)) {
      throw std::invalid_argument("cand_idx_u64 must be contiguous");
    }
  }
  if (!h1_diag_dev.strides_bytes.empty()) {
    if (h1_diag_dev.strides_bytes.size() != 1 || h1_diag_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("h1_diag must be contiguous");
    }
  }
  if (!eri_ppqq_dev.strides_bytes.empty()) {
    if (eri_ppqq_dev.strides_bytes.size() != 1 || eri_ppqq_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("eri_ppqq must be contiguous");
    }
  }
  if (!eri_pqqp_dev.strides_bytes.empty()) {
    if (eri_pqqp_dev.strides_bytes.size() != 1 || eri_pqqp_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("eri_pqqp must be contiguous");
    }
  }
  if (!hdiag_out_dev.strides_bytes.empty()) {
    if (hdiag_out_dev.strides_bytes.size() != 1 || hdiag_out_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
  ...
  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (hdiag_out_dev.stream) stream_u = hdiag_out_dev.stream;
    else if (cand_dev.stream) stream_u = cand_dev.stream;
    else if (h1_diag_dev.stream) stream_u = h1_diag_dev.stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);

  throw_on_cuda_error(
      cas36_diag_guess_candidates_u64_dense_launch_stream(
          drt.child,
          drt.child_prefix,
          ncsf_u64,
          norb,
          reinterpret_cast<const uint64_t*>(cand_dev.ptr),
          ncand,
          reinterpret_cast<const double*>(h1_diag_dev.ptr),
          reinterpret_cast<const double*>(eri_ppqq_dev.ptr),
          reinterpret_cast<const double*>(eri_pqqp_dev.ptr),
          int(neleca),
          int(nelecb),
          reinterpret_cast<double*>(hdiag_out_dev.ptr),
          stream_t,
          threads),
      "cas36_diag_guess_candidates_u64_dense_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cas36_diag_guess_candidates_u64_dense)");
  }
}

void cas36_cipsi_score_pt2_compact_u64_inplace_device(
    py::object idx_u64_obj,
    py::object vals_root_major_obj,
    py::object e_var_obj,
    py::object cand_hdiag_obj,
    py::object selected_idx_sorted_u64_obj,
    double denom_floor,
    py::object score_bits_out_obj,
    py::object pt2_out_obj,
    int threads,
    uint64_t stream,
    bool sync) {
  if (threads <= 0 || threads > 1024) {
  ...
  if (idx_u64_obj.is_none() || vals_root_major_obj.is_none() || e_var_obj.is_none() || cand_hdiag_obj.is_none() ||
      score_bits_out_obj.is_none() || pt2_out_obj.is_none()) {
    throw std::invalid_argument("idx_u64/vals_root_major/e_var/cand_hdiag/score_bits_out/pt2_out must be device arrays");
  }

  auto idx_dev = cuda_array_view_from_object(idx_u64_obj, "idx_u64");
  auto vals_dev = cuda_array_view_from_object(vals_root_major_obj, "vals_root_major");
  auto e_var_dev = cuda_array_view_from_object(e_var_obj, "e_var");
  auto hdiag_dev = cuda_array_view_from_object(cand_hdiag_obj, "cand_hdiag");
  auto score_dev = cuda_array_view_from_object(score_bits_out_obj, "score_bits_out");
  auto pt2_dev = cuda_array_view_from_object(pt2_out_obj, "pt2_out");

  require_typestr(idx_dev, "idx_u64", "<u8");
  require_typestr(vals_dev, "vals_root_major", "<f8");
  require_typestr(e_var_dev, "e_var", "<f8");
  require_typestr(hdiag_dev, "cand_hdiag", "<f8");
  require_typestr(score_dev, "score_bits_out", "<u8");
  require_typestr(pt2_dev, "pt2_out", "<f8");

  if (score_dev.read_only || pt2_dev.read_only) {
    throw std::invalid_argument("score_bits_out and pt2_out must be writable");
  }
  if (idx_dev.shape.size() != 1 || e_var_dev.shape.size() != 1 || hdiag_dev.shape.size() != 1 ||
      score_dev.shape.size() != 1 || pt2_dev.shape.size() != 1) {
    throw std::invalid_argument("idx_u64/e_var/cand_hdiag/score_bits_out/pt2_out must be 1D");
  }
  if (vals_dev.shape.size() != 2) {
    throw std::invalid_argument("vals_root_major must be 2D with shape (nroots, stride)");
  }

  int64_t ncand64 = idx_dev.shape[0];
  if (ncand64 < 0 || ncand64 > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("candidate length out of range");
  }
  int ncand = (int)ncand64;
  int64_t nroots64 = e_var_dev.shape[0];
  if (nroots64 <= 0 || nroots64 > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("nroots out of range");
  }
  int nroots = (int)nroots64;
  int64_t vals_stride = vals_dev.shape[1];
  if (vals_dev.shape[0] != nroots64 || vals_stride < ncand64) {
    throw std::invalid_argument("vals_root_major must have shape (nroots, stride>=ncand)");
  }
  if (hdiag_dev.shape[0] != ncand64 || score_dev.shape[0] != ncand64) {
    throw std::invalid_argument("cand_hdiag and score_bits_out must match idx_u64 length");
  }
  if (pt2_dev.shape[0] != nroots64) {
    throw std::invalid_argument("pt2_out must have shape (nroots,)");
  }
  if (ncand == 0) return;

  if (!idx_dev.strides_bytes.empty()) {
    if (idx_dev.strides_bytes.size() != 1 || idx_dev.strides_bytes[0] != (int64_t)sizeof(uint64_t)) {
      throw std::invalid_argument("idx_u64 must be contiguous");
    }
  }
  if (!vals_dev.strides_bytes.empty()) {
    if (vals_dev.strides_bytes.size() != 2 || vals_dev.strides_bytes[1] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("vals_root_major must be contiguous along last dimension");
    }
  }
  if (!e_var_dev.strides_bytes.empty()) {
    if (e_var_dev.strides_bytes.size() != 1 || e_var_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("e_var must be contiguous");
    }
  }
  if (!hdiag_dev.strides_bytes.empty()) {
    if (hdiag_dev.strides_bytes.size() != 1 || hdiag_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("cand_hdiag must be contiguous");
    }
  }
  if (!score_dev.strides_bytes.empty()) {
    if (score_dev.strides_bytes.size() != 1 || score_dev.strides_bytes[0] != (int64_t)sizeof(uint64_t)) {
      throw std::invalid_argument("score_bits_out must be contiguous");
    }
  }
  if (!pt2_dev.strides_bytes.empty()) {
    if (pt2_dev.strides_bytes.size() != 1 || pt2_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("pt2_out must be contiguous");
    }
  }
  ...
  if (!selected_idx_sorted_u64_obj.is_none()) {
    ...
    if (!selected_dev.strides_bytes.empty()) {
      if (selected_dev.strides_bytes.size() != 1 ||
          selected_dev.strides_bytes[0] != (int64_t)sizeof(uint64_t)) {
  ...
  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (pt2_dev.stream) stream_u = pt2_dev.stream;
    else if (score_dev.stream) stream_u = score_dev.stream;
    else if (vals_dev.stream) stream_u = vals_dev.stream;
    else if (selected_stream) stream_u = selected_stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);
  ...
  throw_on_cuda_error(
      cas36_cipsi_score_pt2_compact_u64_launch_stream(
          reinterpret_cast<const uint64_t*>(idx_dev.ptr),
          reinterpret_cast<const double*>(vals_dev.ptr),
          (int64_t)vals_stride,
          ncand,
          nroots,
          reinterpret_cast<const double*>(e_var_dev.ptr),
          reinterpret_cast<const double*>(hdiag_dev.ptr),
          d_selected_idx,
          nselected,
          double(denom_floor),
          reinterpret_cast<uint64_t*>(score_dev.ptr),
          reinterpret_cast<double*>(pt2_dev.ptr),
          stream_t,
          threads),
      "cas36_cipsi_score_pt2_compact_u64_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cas36_cipsi_score_pt2_compact_u64)");
  }
}
...
void cas36_hb_screen_and_apply_u64_inplace_device(
  ...
  if (label_hi < label_lo || label_hi > ncsf_u64) {
    throw std::invalid_argument("label window must satisfy 0 <= label_lo <= label_hi <= ncsf");
  }
  if (target_mode < 0 || target_mode > 1) {
    throw std::invalid_argument("target_mode must be 0 (external_only) or 1 (selected_only)");
  }
  ...
  const int8_t* d_sym_pq = nullptr;
  if (!sym_pq_allowed_obj.is_none()) {
    auto sym_view = cuda_array_view_from_object(sym_pq_allowed_obj, "sym_pq_allowed");
    d_sym_pq = reinterpret_cast<const int8_t*>(sym_view.ptr);
  }
...
void cas36_hb_emit_tuples_u64_inplace_device(
    const DeviceDRT& drt,
    uint64_t ncsf_u64,
    py::object sel_idx_u64_obj,
    py::object c_bound_obj,
    int nsel,
    py::object h1_pq_obj,
    py::object h1_abs_obj,
    py::object h1_signed_obj,
    ...
    py::object pq_max_v_obj,
    double eps,
    py::object out_keys_u64_obj,
    py::object out_src_obj,
    py::object out_hij_obj,
    int cap,
    uint64_t label_lo,
    uint64_t label_hi,
    py::object selected_idx_sorted_u64_obj,
  ...
  if (drt.norb <= 0 || drt.norb > 64) {
    throw std::invalid_argument("cas36_hb_emit_tuples_u64 requires drt.norb in 1..64");
  }
  if (ncsf_u64 == 0ull) {
    throw std::invalid_argument("ncsf must be > 0");
  }
  ...
  if (nsel < 0 || n_h1 < 0 || cap <= 0) {
    throw std::invalid_argument("nsel, n_h1, and cap must be valid");
  }
  if (target_mode < 0 || target_mode > 1) {
    throw std::invalid_argument("target_mode must be 0 (external_only) or 1 (selected_only)");
  }
  if (sel_idx_u64_obj.is_none() || c_bound_obj.is_none() || h1_pq_obj.is_none() || h1_abs_obj.is_none() ||
      h1_signed_obj.is_none() || pq_ptr_obj.is_none() || rs_idx_obj.is_none() || v_abs_obj.is_none() ||
      v_signed_obj.is_none() || pq_max_v_obj.is_none() || out_keys_u64_obj.is_none() || out_src_obj.is_none() ||
      out_hij_obj.is_none() ||
      out_n_obj.is_none() || overflow_obj.is_none()) {
    throw std::invalid_argument("cas36_hb_emit_tuples_u64 inputs must be device arrays (cannot be None)");
  }

  auto sel_idx_dev = cuda_array_view_from_object(sel_idx_u64_obj, "sel_idx_u64");
  auto c_bound_dev = cuda_array_view_from_object(c_bound_obj, "c_bound");
  auto h1_pq_dev = cuda_array_view_from_object(h1_pq_obj, "h1_pq");
  auto h1_abs_dev = cuda_array_view_from_object(h1_abs_obj, "h1_abs");
  auto h1_signed_dev = cuda_array_view_from_object(h1_signed_obj, "h1_signed");
  auto pq_ptr_dev = cuda_array_view_from_object(pq_ptr_obj, "pq_ptr");
  ...
  auto v_signed_dev = cuda_array_view_from_object(v_signed_obj, "v_signed");
  auto pq_max_v_dev = cuda_array_view_from_object(pq_max_v_obj, "pq_max_v");
  auto out_keys_dev = cuda_array_view_from_object(out_keys_u64_obj, "out_keys_u64");
  auto out_src_dev = cuda_array_view_from_object(out_src_obj, "out_src");
  auto out_hij_dev = cuda_array_view_from_object(out_hij_obj, "out_hij");
  auto out_n_dev = cuda_array_view_from_object(out_n_obj, "out_n");
  auto overflow_dev = cuda_array_view_from_object(overflow_obj, "overflow");

  require_typestr(sel_idx_dev, "sel_idx_u64", "<u8");
  require_typestr(c_bound_dev, "c_bound", "<f8");
  require_typestr(h1_pq_dev, "h1_pq", "<i4");
  require_typestr(h1_abs_dev, "h1_abs", "<f8");
  require_typestr(h1_signed_dev, "h1_signed", "<f8");
  require_typestr(pq_ptr_dev, "pq_ptr", "<i8");
  ...
  require_typestr(v_signed_dev, "v_signed", "<f8");
  require_typestr(pq_max_v_dev, "pq_max_v", "<f8");
  require_typestr(out_keys_dev, "out_keys_u64", "<u8");
  require_typestr(out_src_dev, "out_src", "<i4");
  require_typestr(out_hij_dev, "out_hij", "<f8");
  require_typestr(out_n_dev, "out_n", "<i4");
  require_typestr(overflow_dev, "overflow", "<i4");

  if (out_keys_dev.read_only || out_src_dev.read_only || out_hij_dev.read_only || out_n_dev.read_only || overflow_dev.read_only) {
    throw std::invalid_argument("out_keys_u64/out_src/out_hij/out_n/overflow must be writable");
  }
  if (sel_idx_dev.shape.size() != 1 || c_bound_dev.shape.size() != 1) {
    throw std::invalid_argument("sel_idx_u64 and c_bound must be 1D");
  }
  if (out_keys_dev.shape.size() != 1 || out_src_dev.shape.size() != 1 || out_hij_dev.shape.size() != 1) {
    throw std::invalid_argument("out_keys_u64, out_src, and out_hij must be 1D");
  }
  if (out_n_dev.shape.size() != 1 || out_n_dev.shape[0] != 1 || overflow_dev.shape.size() != 1 || overflow_dev.shape[0] != 1) {
    throw std::invalid_argument("out_n and overflow must have shape (1,)");
  }
  if (out_keys_dev.shape[0] < (int64_t)cap || out_src_dev.shape[0] < (int64_t)cap || out_hij_dev.shape[0] < (int64_t)cap) {
    throw std::invalid_argument("out_keys_u64, out_src, and out_hij length must be >= cap");
  }
  if ((int)sel_idx_dev.shape[0] < nsel || c_bound_dev.shape[0] < (int64_t)nsel) {
    throw std::invalid_argument("sel_idx_u64/c_bound length must be >= nsel");
  }
  ...
  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (out_hij_dev.stream) stream_u = out_hij_dev.stream;
    else if (out_keys_dev.stream) stream_u = out_keys_dev.stream;
    else if (sel_idx_dev.stream) stream_u = sel_idx_dev.stream;
    else if (c_bound_dev.stream) stream_u = c_bound_dev.stream;
    else if (selected_stream) stream_u = selected_stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);

  throw_on_cuda_error(
      cas36_hb_emit_tuples_u64_launch_stream(
          reinterpret_cast<const uint64_t*>(sel_idx_dev.ptr),
          reinterpret_cast<const double*>(c_bound_dev.ptr),
          int(nsel),
          int(drt.norb),
          ncsf_u64,
          reinterpret_cast<const int32_t*>(h1_pq_dev.ptr),
          ...
          drt.node_twos,
          drt.child_prefix,
          reinterpret_cast<uint64_t*>(out_keys_dev.ptr),
          reinterpret_cast<int*>(out_src_dev.ptr),
          reinterpret_cast<double*>(out_hij_dev.ptr),
          int(cap),
          label_lo,
          label_hi,
          d_selected_idx,
          int(nselected),
          int(target_mode),
          reinterpret_cast<int*>(out_n_dev.ptr),
          reinterpret_cast<int*>(overflow_dev.ptr),
          stream_t,
          int(threads),
          d_sym_pq),
      "cas36_hb_emit_tuples_u64_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cas36_hb_emit_tuples_u64)");
  }
}

void cas36_exact_selected_emit_tuples_u64_inplace_device(
    const DeviceDRT& drt,
    uint64_t ncsf_u64,
    py::object sel_idx_u64_obj,
    py::object c_bound_obj,
    ...
    bool sync) {
  if (selected_idx_sorted_u64_obj.is_none()) {
    throw std::invalid_argument("selected_idx_sorted_u64 must be provided for exact selected tuple emission");
  }
  cas36_hb_emit_tuples_u64_inplace_device(
      drt,
      ncsf_u64,
      std::move(sel_idx_u64_obj),
      std::move(c_bound_obj),
      int(nsel),
      std::move(h1_pq_obj),
      std::move(h1_abs_obj),
      std::move(h1_signed_obj),
      int(n_h1),
      std::move(pq_ptr_obj),
      std::move(rs_idx_obj),
      std::move(v_abs_obj),
      std::move(v_signed_obj),
      std::move(pq_max_v_obj),
      0.0,
      std::move(out_keys_u64_obj),
      std::move(out_src_obj),
      std::move(out_hij_obj),
      int(cap),
      0,
      std::numeric_limits<uint64_t>::max(),
      std::move(selected_idx_sorted_u64_obj),
      1,
      std::move(out_n_obj),
      std::move(overflow_obj),
      py::none(),
      int(threads),
      stream,
      bool(sync));
...
void cas36_exact_selected_emit_tuples_dense_u64_inplace_device(
    const DeviceDRT& drt,
    uint64_t ncsf_u64,
    py::object sel_idx_u64_obj,
    py::object c_bound_obj,
    ...
    py::object out_hij_obj,
    int cap,
    py::object membership_hash_keys_obj,
    int membership_hash_cap,
    py::object out_diag_obj,
    py::object out_n_obj,
    py::object overflow_obj,
    int threads,
    uint64_t stream,
    bool sync) {
  if (sel_idx_u64_obj.is_none() || c_bound_obj.is_none() || h_base_obj.is_none() || eri4_obj.is_none() ||
      out_keys_u64_obj.is_none() || out_src_obj.is_none() || out_hij_obj.is_none() || out_n_obj.is_none() ||
      overflow_obj.is_none()) {
  ...
  if (nsel < 0 || cap <= 0) {
    throw std::invalid_argument("nsel must be >= 0 and cap must be > 0");
  }
  if (drt.norb <= 0 || drt.norb > 32) {
    throw std::invalid_argument("exact selected dense tuple emitter currently supports 1 <= norb <= 32");
  }
  if (nsel == 0) return;
  ...
  auto out_n_dev = cuda_array_view_from_object(out_n_obj, "out_n");
  auto overflow_dev = cuda_array_view_from_object(overflow_obj, "overflow");
  std::optional<CudaArrayView> membership_hash_dev;
  if (!membership_hash_keys_obj.is_none()) {
    membership_hash_dev = cuda_array_view_from_object(membership_hash_keys_obj, "membership_hash_keys");
  }
  std::optional<CudaArrayView> out_diag_dev;
  if (!out_diag_obj.is_none()) {
    out_diag_dev = cuda_array_view_from_object(out_diag_obj, "out_diag");
  }
  ...
  require_typestr(out_n_dev, "out_n", "<i4");
  require_typestr(overflow_dev, "overflow", "<i4");
  if (membership_hash_dev.has_value()) {
    require_typestr(*membership_hash_dev, "membership_hash_keys", "<u8");
  }
  if (out_diag_dev.has_value()) {
    require_typestr(*out_diag_dev, "out_diag", "<f8");
  }

  if (sel_idx_dev.read_only || c_bound_dev.read_only || h_base_dev.read_only || eri4_dev.read_only) {
    // inputs are fine if read-only; only outputs must be writable
  }
  if (out_keys_dev.read_only || out_src_dev.read_only || out_hij_dev.read_only || out_n_dev.read_only || overflow_dev.read_only ||
      (out_diag_dev.has_value() && out_diag_dev->read_only)) {
    throw std::invalid_argument("out_keys_u64/out_src/out_hij/out_n/overflow must be writable");
  }
  if (sel_idx_dev.shape.size() != 1 || c_bound_dev.shape.size() != 1 || h_base_dev.shape.size() != 1 || eri4_dev.shape.size() != 1 ||
      out_keys_dev.shape.size() != 1 || out_src_dev.shape.size() != 1 || out_hij_dev.shape.size() != 1 ||
      out_n_dev.shape.size() != 1 || overflow_dev.shape.size() != 1) {
    throw std::invalid_argument("all exact selected dense tuple emitter arrays must be 1D");
  }
  if (out_diag_dev.has_value() && out_diag_dev->shape.size() != 1) {
    throw std::invalid_argument("out_diag must be a 1D device array");
  }
  if (sel_idx_dev.shape[0] < (int64_t)nsel || c_bound_dev.shape[0] < (int64_t)nsel) {
    throw std::invalid_argument("sel_idx_u64/c_bound length must be >= nsel");
  }
  int64_t nops = (int64_t)drt.norb * (int64_t)drt.norb;
  int64_t eri4_n = nops * nops;
  if (h_base_dev.shape[0] < nops) {
    throw std::invalid_argument("h_base length is smaller than norb*norb");
  }
  if (eri4_dev.shape[0] < eri4_n) {
    throw std::invalid_argument("eri4 length is smaller than norb^4");
  }
  if (out_keys_dev.shape[0] < (int64_t)cap || out_src_dev.shape[0] < (int64_t)cap || out_hij_dev.shape[0] < (int64_t)cap) {
    throw std::invalid_argument("out_keys_u64, out_src, and out_hij length must be >= cap");
  }
  if (out_n_dev.shape[0] != 1 || overflow_dev.shape[0] != 1) {
    throw std::invalid_argument("out_n and overflow must have shape (1,)");
  }
  if (out_diag_dev.has_value() && out_diag_dev->shape[0] < (int64_t)nsel) {
    throw std::invalid_argument("out_diag length must be >= nsel");
  }
  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (out_hij_dev.stream) stream_u = out_hij_dev.stream;
    ...
    else if (eri4_dev.stream) stream_u = eri4_dev.stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);

  throw_on_cuda_error(
      cas36_exact_selected_emit_tuples_dense_u64_launch_stream(
          reinterpret_cast<const uint64_t*>(sel_idx_dev.ptr),
          reinterpret_cast<const double*>(c_bound_dev.ptr),
          int(nsel),
          int(drt.norb),
          ...
          reinterpret_cast<double*>(out_hij_dev.ptr),
          int(cap),
          membership_hash_dev.has_value() ? reinterpret_cast<const uint64_t*>(membership_hash_dev->ptr) : nullptr,
          membership_hash_dev.has_value() ? int(membership_hash_cap) : 0,
          out_diag_dev.has_value() ? reinterpret_cast<double*>(out_diag_dev->ptr) : nullptr,
          reinterpret_cast<int*>(out_n_dev.ptr),
          reinterpret_cast<int*>(overflow_dev.ptr),
          stream_t,
          int(threads)),
  ...
  if (sync) {
    throw_on_cuda_error(
        cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(cas36_exact_selected_emit_tuples_dense_u64)");
  }
}

void cas36_sci_build_membership_hash_u64_inplace_device(
    py::object sorted_keys_obj,
    int nkeys,
    py::object hash_keys_obj,
    int cap,
    uint64_t stream,
    bool sync) {
  if (sorted_keys_obj.is_none() || hash_keys_obj.is_none()) {
    throw std::invalid_argument("sorted_keys and hash_keys must not be None");
  }
  if (nkeys <= 0 || cap <= 0) return;
  auto sorted_dev = cuda_array_view_from_object(sorted_keys_obj, "sorted_keys");
  auto hash_dev = cuda_array_view_from_object(hash_keys_obj, "hash_keys");
  require_typestr(sorted_dev, "sorted_keys", "<u8");
  require_typestr(hash_dev, "hash_keys", "<u8");
  if (sorted_dev.shape.size() != 1 || hash_dev.shape.size() != 1) {
    throw std::invalid_argument("sorted_keys and hash_keys must be 1D");
  }
  if (sorted_dev.shape[0] < (int64_t)nkeys) {
    throw std::invalid_argument("sorted_keys length must be >= nkeys");
  }
  if (hash_dev.shape[0] < (int64_t)cap) {
    throw std::invalid_argument("hash_keys length must be >= cap");
  }
  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (sorted_dev.stream) stream_u = sorted_dev.stream;
    else if (hash_dev.stream) stream_u = hash_dev.stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);
  throw_on_cuda_error(
      cas36_sci_build_membership_hash_u64_launch_stream(
          reinterpret_cast<const uint64_t*>(sorted_dev.ptr),
          int(nkeys),
          reinterpret_cast<uint64_t*>(hash_dev.ptr),
          int(cap),
          stream_t),
      "cas36_sci_build_membership_hash_u64_launch_stream");
  if (sync) {
    throw_on_cuda_error(
        cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(build_membership_hash)");
  }
}

void pairwise_materialize_u64_inplace_device(
    const DeviceDRT& drt,
    uint64_t ncsf_u64,
    py::object sel_idx_u64_obj,
    int nsel,
    ...
    uint64_t stream,
    bool sync) {
  if (sel_idx_u64_obj.is_none() || steps_all_obj.is_none() || nodes_all_obj.is_none() ||
      occ_all_obj.is_none() || b_all_obj.is_none() || overflow_obj.is_none()) {
    throw std::invalid_argument("required array arguments must not be None");
  }
  ...
  uint64_t stream_u = stream;
  if (stream_u == 0) {
    if (steps_dev.stream) stream_u = steps_dev.stream;
    else if (sel_dev.stream) stream_u = sel_dev.stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);

  throw_on_cuda_error(
      pairwise_materialize_u64_launch_stream(
          ...
          drt.node_twos,
          drt.child_prefix,
          reinterpret_cast<int8_t*>(steps_dev.ptr),
          reinterpret_cast<int32_t*>(nodes_dev.ptr),
          reinterpret_cast<int8_t*>(occ_dev.ptr),
          reinterpret_cast<int16_t*>(b_dev.ptr),
          reinterpret_cast<int*>(overflow_dev.ptr),
          stream_t,
          int(threads)),
      "pairwise_materialize_u64_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(pairwise_materialize)");
  }
}

void pairwise_hij_u64_inplace_device(
    const DeviceDRT& drt,
    uint64_t ncsf_u64,
    py::object sel_idx_u64_obj,
    int nsel,
  ...
  throw_on_cuda_error(
      pairwise_hij_u64_launch_stream(
          reinterpret_cast<const uint64_t*>(sel_dev.ptr),
          int(nsel),
          int(drt.norb),
          ncsf_u64,
      ...
      "pairwise_hij_u64_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(pairwise_hij)");
  }
}
...
void hb_screen_and_apply_many_roots_inplace_device(
    const DeviceDRT& drt,
    const DeviceStateCache& state,
    py::object sel_idx_obj,
    py::object c_sel_obj,
    ...
    uint64_t stream,
    bool sync) {
  if (state.steps == nullptr) throw std::runtime_error("DeviceStateCache is not initialized");
  if (drt.norb != state.norb) throw std::invalid_argument("DeviceDRT and DeviceStateCache have inconsistent norb");
  if (drt.norb <= 0 || drt.norb > MAX_NORB) throw std::invalid_argument("norb out of supported range");
  if (threads <= 0 || threads > 1024) throw std::invalid_argument("threads must be in 1..1024");
  if (nsel <= 0 || nroots <= 0) return;

  auto sel_idx_dev = cuda_array_view_from_object(sel_idx_obj, "sel_idx");
  auto c_sel_dev = cuda_array_view_from_object(c_sel_obj, "c_sel");
  auto h1_pq_dev = cuda_array_view_from_object(h1_pq_obj, "h1_pq");
  auto h1_abs_dev = cuda_array_view_from_object(h1_abs_obj, "h1_abs");
  auto h1_signed_dev = cuda_array_view_from_object(h1_signed_obj, "h1_signed");
  auto pq_ptr_dev = cuda_array_view_from_object(pq_ptr_obj, "pq_ptr");
  ...
  require_typestr(sel_idx_dev, "sel_idx", "<i4");
  require_typestr(c_sel_dev, "c_sel", "<f8");
  require_typestr(h1_pq_dev, "h1_pq", "<i4");
  require_typestr(h1_abs_dev, "h1_abs", "<f8");
  require_typestr(h1_signed_dev, "h1_signed", "<f8");
  require_typestr(pq_ptr_dev, "pq_ptr", "<i8");
  ...
  if (sel_idx_dev.shape.size() != 1 || sel_idx_dev.shape[0] != (int64_t)nsel) {
    throw std::invalid_argument("sel_idx must have shape (nsel,)");
  }
  if (c_sel_dev.shape.size() != 2 || c_sel_dev.shape[0] != (int64_t)nsel || c_sel_dev.shape[1] != (int64_t)nroots) {
    throw std::invalid_argument("c_sel must have shape (nsel, nroots)");
  }
  if (!c_sel_dev.strides_bytes.empty()) {
    if (c_sel_dev.strides_bytes.size() != 2 ||
        c_sel_dev.strides_bytes[1] != (int64_t)sizeof(double) ||
        c_sel_dev.strides_bytes[0] != (int64_t)nroots * (int64_t)sizeof(double)) {
      throw std::invalid_argument("c_sel must be C-contiguous");
    }
  }
  ...
  if (stream_u == 0) {
    if (sel_idx_dev.stream) stream_u = sel_idx_dev.stream;
    else if (c_sel_dev.stream) stream_u = c_sel_dev.stream;
    else if (selected_stream) stream_u = selected_stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);

  throw_on_cuda_error(
      guga_hb_screen_and_apply_many_roots_launch_stream(
          reinterpret_cast<const int32_t*>(sel_idx_dev.ptr),
          reinterpret_cast<const double*>(c_sel_dev.ptr),
          nsel,
          nroots,
          state.steps,
          state.nodes,
      ...
      "guga_hb_screen_and_apply_many_roots_launch_stream");
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(hb_screen_and_apply_many_roots)");
  }
}
...
void build_w_from_epq_transpose_range_mm_scaled_inplace_device(
    const DeviceStateCache& state,
    py::object epq_t_indptr,
    py::object epq_t_source,
    py::object epq_t_pq,
  ...
  if (hdiag.is_none()) {
    throw std::invalid_argument("hdiag must be a device array (cannot be None)");
  }
  if (epsa.is_none()) {
    throw std::invalid_argument("epsa must be a device array (cannot be None)");
  }
  if (k_start < 0 || k_start >= state.ncsf) {
    throw std::invalid_argument("k_start out of range");
  }
  ...
  auto epq_t_data_dev = cuda_array_view_from_object(epq_t_data, "epq_t_data");
  const std::string epq_t_data_t = normalize_typestr(epq_t_data_dev.typestr);
  bool epq_use_f32 = false;
  if (epq_t_data_t == "<f4") epq_use_f32 = true;
  else if (epq_t_data_t == "<f8") epq_use_f32 = false;
  else throw std::invalid_argument("epq_t_data must have typestr <f4 or <f8");
  if (epq_t_data_dev.shape != epq_t_source_dev.shape) {
    throw std::invalid_argument("epq_t_data must have the same shape as epq_t_source");
  }
  int64_t epq_data_itemsize = epq_use_f32 ? (int64_t)sizeof(float) : (int64_t)sizeof(double);
  if (!epq_t_data_dev.strides_bytes.empty()) {
    if (epq_t_data_dev.strides_bytes.size() != 1 || epq_t_data_dev.strides_bytes[0] != epq_data_itemsize) {
      throw std::invalid_argument("epq_t_data must be contiguous");
    }
  ...
  CudaArrayView x_dev = cuda_array_view_from_object(x, "x");
  require_typestr(x_dev, "x", "<f8");
  if (x_dev.shape.size() != 2 || x_dev.shape[0] != (int64_t)state.ncsf) {
    throw std::invalid_argument("x must have shape (ncsf,nvec)");
  }
  int64_t nvec64 = x_dev.shape[1];
  if (nvec64 <= 0 || nvec64 > 32) {
    throw std::invalid_argument("x nvec must be in 1..32");
  }
  int nvec = (int)nvec64;
  int64_t x_stride = nvec64;
  if (x_dev.strides_bytes.empty()) {
    x_stride = nvec64;
  } else {
    if (x_dev.strides_bytes.size() != 2) throw std::invalid_argument("x must be 2D with valid strides");
    int64_t s0 = x_dev.strides_bytes[0];
    int64_t s1 = x_dev.strides_bytes[1];
    if (s0 <= 0 || s1 <= 0) throw std::invalid_argument("x must have positive strides");
    if (s1 != (int64_t)sizeof(double)) throw std::invalid_argument("x must be contiguous along last dimension");
    if (s0 % (int64_t)sizeof(double) != 0) throw std::invalid_argument("x row stride must be a multiple of itemsize");
    if (s0 < (int64_t)nvec * (int64_t)sizeof(double)) throw std::invalid_argument("x row stride too small");
    x_stride = s0 / (int64_t)sizeof(double);
  }

  CudaArrayView hdiag_dev = cuda_array_view_from_object(hdiag, "hdiag");
  require_typestr(hdiag_dev, "hdiag", "<f8");
  if (hdiag_dev.shape.size() != 1 || hdiag_dev.shape[0] != (int64_t)state.ncsf) {
    throw std::invalid_argument("hdiag must have shape (ncsf,)");
  }
  if (!hdiag_dev.strides_bytes.empty()) {
    if (hdiag_dev.strides_bytes.size() != 1 || hdiag_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("hdiag must be contiguous");
  ...
  CudaArrayView epsa_dev = cuda_array_view_from_object(epsa, "epsa");
  require_typestr(epsa_dev, "epsa", "<f8");
  if (epsa_dev.shape.size() != 1 || epsa_dev.shape[0] != (int64_t)state.norb) {
    throw std::invalid_argument("epsa must have shape (norb,)");
  }
  if (!epsa_dev.strides_bytes.empty()) {
    if (epsa_dev.strides_bytes.size() != 1 || epsa_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
      throw std::invalid_argument("epsa must be contiguous");
    }
  }
  ...
  int64_t w_stride = 0;
  int64_t out_cols = nops_ll * (int64_t)nvec;
  if (w_out_dev.shape.size() == 1) {
    if (w_out_dev.shape[0] != (int64_t)eff_k_count * out_cols) {
      throw std::invalid_argument("w_out (1D) must have shape (k_count*nops*nvec,)");
    }
    if (!w_out_dev.strides_bytes.empty()) {
      if (w_out_dev.strides_bytes.size() != 1 || w_out_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
        throw std::invalid_argument("w_out (1D) must be contiguous");
    ...
    if (w_out_dev.shape[0] != (int64_t)eff_k_count) {
      throw std::invalid_argument("w_out (2D) must have shape (k_count, nops*nvec_or_more)");
    }
    if (w_out_dev.shape[1] < out_cols) {
      throw std::invalid_argument("w_out (2D) second dimension too small");
    }
    if (w_out_dev.strides_bytes.empty()) {
      w_stride = w_out_dev.shape[1];
  ...
  if (stream_u == 0) {
    ...
    else if (epsa_dev.stream) stream_u = epsa_dev.stream;
    else if (epq_t_data_dev.stream) stream_u = epq_t_data_dev.stream;
    else if (overflow_dev.stream) stream_u = overflow_dev.stream;
  }
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream_u);
  ...
  const double* d_x_f64 = reinterpret_cast<const double*>(x_dev.ptr);
  const double* d_hdiag = reinterpret_cast<const double*>(hdiag_dev.ptr);
  const double* d_epsa = reinterpret_cast<const double*>(epsa_dev.ptr);
  double* d_w_out = reinterpret_cast<double*>(w_out_dev.ptr);
  int* d_overflow = reinterpret_cast<int*>(overflow_dev.ptr);
  ...
  if (epq_use_f32) {
    guga_build_w_from_epq_transpose_range_mm_scaled_f64_out_f32_coeff_launch_stream(
        state.steps,
        int(state.ncsf),
        int(state.norb),
        d_epq_t_indptr,
    ...
    guga_build_w_from_epq_transpose_range_mm_scaled_launch_stream(
        state.steps,
        int(state.ncsf),
        int(state.norb),
        d_epq_t_indptr,
  ...
  throw_on_cuda_error(cudaGetLastError(), "kernel launch(build_w_from_epq_transpose_range_mm_scaled)");
  ...
  if (sync) {
    throw_on_cuda_error(
        cudaStreamSynchronize(stream_t),
        "cudaStreamSynchronize(build_w_from_epq_transpose_range_mm_scaled)");
  }
  if (check_overflow && h_overflow) {
    throw std::runtime_error("build_w_from_epq_transpose_range_mm_scaled overflow (invalid indices)");
  }
}

void build_w_from_epq_transpose_range_mm_inplace_device(
    const DeviceStateCache& state,
    py::object epq_t_indptr,
    py::object epq_t_source,
    py::object epq_t_pq,
  ...
  if (x_dev.strides_bytes.empty()) {
    ...
    int64_t s1 = x_dev.strides_bytes[1];
    if (s0 <= 0 || s1 <= 0) throw std::invalid_argument("x must have positive strides");
    if (s1 != (int64_t)sizeof(double)) throw std::invalid_argument("x must be dense along last dimension");
    if (s0 % (int64_t)sizeof(double) != 0) throw std::invalid_argument("x row stride must be a multiple of itemsize");
    x_stride = s0 / (int64_t)sizeof(double);
    if (x_stride < nvec64) throw std::invalid_argument("x row stride too small");
  }

  CudaArrayView w_out_dev = cuda_array_view_from_object(w_out, "w_out");
  require_typestr(w_out_dev, "w_out", "<f8");
  if (w_out_dev.read_only) {
    throw std::invalid_argument("w_out must be writable");
  }
  ...
  if (epq_use_f32) {
    guga_build_w_from_epq_transpose_range_mm_f64_out_f32_coeff_launch_stream(
        state.steps,
        int(state.ncsf),
        int(state.norb),
        d_epq_t_indptr,
    ...
    guga_build_w_from_epq_transpose_range_mm_launch_stream(
        state.steps,
        int(state.ncsf),
        int(state.norb),
        d_epq_t_indptr,
  ...
  throw_on_cuda_error(cudaGetLastError(), "kernel launch(build_w_from_epq_transpose_range_mm)");
  ...
  if (sync) {
    throw_on_cuda_error(
        cudaStreamSynchronize(stream_t),
        "cudaStreamSynchronize(build_w_from_epq_transpose_range_mm)");
  }
  if (check_overflow && h_overflow) {
    throw std::runtime_error("build_w_from_epq_transpose_range_mm overflow (invalid indices)");
  }
}
...
void qmc_spawn_hamiltonian_inplace_device_initiator_dev(
    const DeviceDRT& drt,
    const DeviceStateCache& state,
    py::object x_idx,
    py::object x_val,
  ...
  const double* d_initiator_t = nullptr;
  if (!initiator_t_dev.is_none()) {
    CudaArrayView t_dev = cuda_array_view_from_object(initiator_t_dev, "initiator_t_dev");
    require_typestr(t_dev, "initiator_t_dev", "<f8");
    if (t_dev.shape.size() == 1) {
      if (t_dev.shape[0] != 1) throw std::invalid_argument("initiator_t_dev must have shape (1,) or ()");
      if (!t_dev.strides_bytes.empty()) {
        if (t_dev.strides_bytes.size() != 1) throw std::invalid_argument("initiator_t_dev must be 1D with valid strides");
        if (t_dev.strides_bytes[0] != (int64_t)sizeof(double)) throw std::invalid_argument("initiator_t_dev must be contiguous");
      }
    } else if (t_dev.shape.size() == 0) {
      // scalar: ok
    } else {
      throw std::invalid_argument("initiator_t_dev must have shape (1,) or ()");
    }
    d_initiator_t = (const double*)t_dev.ptr;
  }
  ...
  throw_on_cuda_error(
      guga_qmc_spawn_hamiltonian_initiator_dev_launch_stream(
          drt.child,
          drt.node_twos,
          drt.child_prefix,
          state.steps,
  ...
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_spawn_hamiltonian_initiator_dev)");
  }
}
...
void qmc_spawn_hamiltonian_u64_inplace_device_initiator_dev(
    const DeviceDRT& drt,
    py::object x_key,
    py::object x_val,
    py::object h_base_flat,
  ...
  if (initiator_t_dev.is_none()) {
    throw std::invalid_argument("initiator_t_dev must be a device scalar (cannot be None)");
  }
  CudaArrayView t_dev = cuda_array_view_from_object(initiator_t_dev, "initiator_t_dev");
  require_typestr(t_dev, "initiator_t_dev", "<f8");
  if (t_dev.shape.size() == 1) {
  ...
  const double* d_initiator_t = (const double*)t_dev.ptr;
  ...
  if (m64 > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("x_key length out of supported range");
  }
  int m = (int)m64;
  ...
  int64_t nspawn_total = (int64_t)nspawn_one + (int64_t)nspawn_two;
  int64_t out_len64 = m64 * nspawn_total;
  if (out_len64 < 0 || out_len64 > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("invalid output length");
  }
  if (out_key_dev.shape[0] != out_len64 || out_val_dev.shape[0] != out_len64) {
    throw std::invalid_argument("out_key/out_val must have shape (len(x_key)*(nspawn_one+nspawn_two),)");
  }
  if (m == 0) return;
  ...
  throw_on_cuda_error(
      guga_qmc_spawn_hamiltonian_u64_f64_initiator_dev_launch_stream(
          drt.child,
          drt.node_twos,
          drt.norb,
          (const uint64_t*)x_key_dev.ptr,
  ...
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_spawn_hamiltonian_u64_initiator_dev)");
  }
}

void qmc_spawn_hamiltonian_idx64_u64_inplace_device(
    const DeviceDRT& drt,
    uint64_t ncsf_u64,
    py::object x_idx_u64,
    py::object x_val,
  ...
  if (drt.norb <= 0 || drt.norb > 64) {
    throw std::invalid_argument("idx64 QMC kernels require norb in 1..64");
  }
  if (ncsf_u64 == 0ull) {
    throw std::invalid_argument("ncsf must be > 0 for idx64 QMC spawn");
  }
  if (threads <= 0 || threads > 1024) {
    throw std::invalid_argument("threads must be in 1..1024");
  }
  ...
  if (x_idx_u64.is_none() || x_val.is_none() || h_base_flat.is_none() || eri_mat.is_none() || out_idx_u64.is_none() ||
      out_val.is_none()) {
    throw std::invalid_argument(
        "x_idx_u64/x_val/h_base_flat/eri_mat/out_idx_u64/out_val must be device arrays (cannot be None)");
  }

  CudaArrayView x_idx_dev = cuda_array_view_from_object(x_idx_u64, "x_idx_u64");
  CudaArrayView x_val_dev = cuda_array_view_from_object(x_val, "x_val");
  CudaArrayView h_base_dev = cuda_array_view_from_object(h_base_flat, "h_base_flat");
  CudaArrayView eri_dev = cuda_array_view_from_object(eri_mat, "eri_mat");
  CudaArrayView out_idx_dev = cuda_array_view_from_object(out_idx_u64, "out_idx_u64");
  CudaArrayView out_val_dev = cuda_array_view_from_object(out_val, "out_val");

  require_typestr(x_idx_dev, "x_idx_u64", "<u8");
  require_typestr(x_val_dev, "x_val", "<f8");
  require_typestr(h_base_dev, "h_base_flat", "<f8");
  require_typestr(eri_dev, "eri_mat", "<f8");
  require_typestr(out_idx_dev, "out_idx_u64", "<u8");
  require_typestr(out_val_dev, "out_val", "<f8");

  if (out_idx_dev.read_only || out_val_dev.read_only) {
    throw std::invalid_argument("out_idx_u64/out_val must be writable");
  }

  if (x_idx_dev.shape.size() != 1 || x_val_dev.shape.size() != 1) {
    throw std::invalid_argument("x_idx_u64 and x_val must be 1D arrays");
  }
  if (h_base_dev.shape.size() != 1) {
    throw std::invalid_argument("h_base_flat must be a 1D array");
  }
  ...
  if (out_idx_dev.shape.size() != 1 || out_val_dev.shape.size() != 1) {
    throw std::invalid_argument("out_idx_u64 and out_val must be 1D arrays");
  }

  int64_t m64 = x_idx_dev.shape[0];
  if (m64 < 0) {
    throw std::invalid_argument("x_idx_u64 length must be non-negative");
  }
  if (x_val_dev.shape[0] != m64) {
    throw std::invalid_argument("x_idx_u64 and x_val must have the same length");
  }
  if (m64 > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("x_idx_u64 length out of supported range");
  }
  int m = (int)m64;
  ...
  if (out_idx_dev.shape[0] != out_len64 || out_val_dev.shape[0] != out_len64) {
    throw std::invalid_argument("out_idx_u64/out_val must have shape (len(x_idx_u64)*(nspawn_one+nspawn_two),)");
  }
  if (m == 0) return;

  if (!x_idx_dev.strides_bytes.empty()) {
    if (x_idx_dev.strides_bytes.size() != 1 || x_idx_dev.strides_bytes[0] != (int64_t)sizeof(uint64_t)) {
      throw std::invalid_argument("x_idx_u64 must be contiguous");
    }
  }
  if (!x_val_dev.strides_bytes.empty()) {
    if (x_val_dev.strides_bytes.size() != 1 || x_val_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
  ...
  if (!out_idx_dev.strides_bytes.empty()) {
    if (out_idx_dev.strides_bytes.size() != 1 || out_idx_dev.strides_bytes[0] != (int64_t)sizeof(uint64_t)) {
      throw std::invalid_argument("out_idx_u64 must be contiguous");
    }
  }
  if (!out_val_dev.strides_bytes.empty()) {
    if (out_val_dev.strides_bytes.size() != 1 || out_val_dev.strides_bytes[0] != (int64_t)sizeof(double)) {
  ...
  if (pair_sampling_mode_in != 0) {
    ...
    if (pair_alias_prob_dev.shape.size() != 1 || pair_alias_idx_dev.shape.size() != 1 ||
        pair_norm_dev.shape.size() != 1) {
      throw std::invalid_argument("pair_alias_prob/pair_alias_idx/pair_norm must be 1D arrays");
    }
    if (pair_alias_prob_dev.shape[0] != nops64 || pair_alias_idx_dev.shape[0] != nops64 ||
        pair_norm_dev.shape[0] != nops64) {
      throw std::invalid_argument("pair_alias_prob/pair_alias_idx/pair_norm must have length norb*norb");
    }
    if (!pair_alias_prob_dev.strides_bytes.empty()) {
      if (pair_alias_prob_dev.strides_bytes.size() != 1 ||
          pair_alias_prob_dev.strides_bytes[0] != (int64_t)sizeof(float)) {
        throw std::invalid_argument("pair_alias_prob must be contiguous");
      }
    }
    if (!pair_alias_idx_dev.strides_bytes.empty()) {
      if (pair_alias_idx_dev.strides_bytes.size() != 1 ||
          pair_alias_idx_dev.strides_bytes[0] != (int64_t)sizeof(int32_t)) {
        throw std::invalid_argument("pair_alias_idx must be contiguous");
      }
    }
    if (!pair_norm_dev.strides_bytes.empty()) {
  ...
  cudaStream_t stream_t = reinterpret_cast<cudaStream_t>(stream);
  throw_on_cuda_error(
      cudaMemsetAsync(out_idx_dev.ptr, 0xFF, (size_t)out_len64 * sizeof(uint64_t), stream_t),
      "cudaMemsetAsync(out_idx_u64=INVALID)");
  throw_on_cuda_error(
      cudaMemsetAsync(out_val_dev.ptr, 0, (size_t)out_len64 * sizeof(double), stream_t),
      "cudaMemsetAsync(out_val=0)");

  throw_on_cuda_error(
      guga_qmc_spawn_hamiltonian_idx64_u64_f64_launch_stream(
          drt.child,
          drt.node_twos,
          drt.child_prefix,
          ncsf_u64,
          drt.norb,
          (const uint64_t*)x_idx_dev.ptr,
          (const double*)x_val_dev.ptr,
          m,
          (const double*)h_base_dev.ptr,
          (const double*)eri_dev.ptr,
  ...
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_spawn_hamiltonian_idx64_u64)");
  }
}

void qmc_spawn_hamiltonian_idx64_u64_inplace_device_initiator_dev(
    const DeviceDRT& drt,
    uint64_t ncsf_u64,
    py::object x_idx_u64,
    py::object x_val,
  ...
  throw_on_cuda_error(
      guga_qmc_spawn_hamiltonian_idx64_u64_f64_initiator_dev_launch_stream(
          drt.child,
          drt.node_twos,
          drt.child_prefix,
          ncsf_u64,
  ...
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_spawn_hamiltonian_idx64_u64_initiator_dev)");
  }
}
...
void qmc_coalesce_coo_u64_f64_inplace_device(
    py::object key_in,
    py::object val_in,
    py::object key_out,
    py::object val_out,
  ...
  int64_t n64 = key_in_dev.shape[0];
  if (n64 < 0 || n64 > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("input length out of supported range for u64 kernel interface");
  }
  int n = (int)n64;
  ...
  if (key_out_dev.shape[0] < n64 || val_out_dev.shape[0] < n64) {
    throw std::invalid_argument("key_out/val_out must have length >= len(key_in)");
  }
  ...
  cudaStream_t stream_t = (cudaStream_t)stream;
  throw_on_cuda_error(
      guga_qmc_coalesce_coo_u64_f64_launch_stream(
          (const uint64_t*)key_in_dev.ptr,
          (const double*)val_in_dev.ptr,
          n,
          (uint64_t*)key_out_dev.ptr,
  ...
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_coalesce_coo_u64)");
  }
}
...
void qmc_phi_pivot_resample_u64_f64_inplace_device(
    py::object key_in,
    py::object val_in,
    py::object key_out,
    py::object val_out,
  ...
  int64_t n_in64 = key_in_dev.shape[0];
  if (n_in64 < 0 || n_in64 > (int64_t)std::numeric_limits<int>::max()) {
    throw std::invalid_argument("input length out of supported range for u64 kernel interface");
  }
  int n_in = (int)n_in64;
  ...
  cudaStream_t stream_t = (cudaStream_t)stream;
  throw_on_cuda_error(
      guga_qmc_phi_pivot_resample_u64_f64_launch_stream(
          (const uint64_t*)key_in_dev.ptr,
          (const double*)val_in_dev.ptr,
          n_in,
          (uint64_t*)key_out_dev.ptr,
  ...
  if (sync) {
    throw_on_cuda_error(cudaStreamSynchronize(stream_t), "cudaStreamSynchronize(qmc_phi_pivot_resample_u64)");
  }
}
...
PYBIND11_MODULE(_guga_cuda_ext, m) {
  ...
  m.def(
      "qmc_spawn_hamiltonian_inplace_device_initiator_dev",
      &qmc_spawn_hamiltonian_inplace_device_initiator_dev,
      py::arg("drt"),
      py::arg("state"),
      py::arg("x_idx"),
      ...
      py::arg("nspawn_two"),
      py::arg("seed"),
      py::arg("initiator_t_dev") = py::none(),
      py::arg("threads") = 128,
      py::arg("stream") = 0,
      py::arg("sync") = true);
  ...
  m.def(
      "qmc_spawn_hamiltonian_u64_inplace_device_initiator_dev",
      &qmc_spawn_hamiltonian_u64_inplace_device_initiator_dev,
      py::arg("drt"),
      py::arg("x_key"),
      py::arg("x_val"),
  ...
  m.def(
      "qmc_spawn_hamiltonian_idx64_u64_inplace_device",
      &qmc_spawn_hamiltonian_idx64_u64_inplace_device,
      py::arg("drt"),
      py::arg("ncsf"),
      py::arg("x_idx_u64"),
  ...
  m.def(
      "qmc_spawn_hamiltonian_idx64_u64_inplace_device_initiator_dev",
      &qmc_spawn_hamiltonian_idx64_u64_inplace_device_initiator_dev,
      py::arg("drt"),
      py::arg("ncsf"),
      py::arg("x_idx_u64"),
  ...
  m.def(
      "qmc_coalesce_coo_u64_f64_inplace_device",
      &qmc_coalesce_coo_u64_f64_inplace_device,
      py::arg("key_in"),
      py::arg("val_in"),
      py::arg("key_out"),
  ...
  m.def(
      "qmc_phi_pivot_resample_u64_f64_inplace_device",
      &qmc_phi_pivot_resample_u64_f64_inplace_device,
      py::arg("key_in"),
      py::arg("val_in"),
      py::arg("key_out"),
  ...
  py::class_<QmcWorkspaceU64>(m, "QmcWorkspaceU64")
      .def(py::init<int, int>(), py::arg("max_n"), py::arg("max_m"))
      .def("release", &QmcWorkspaceU64::release)
      .def_property_readonly("max_n", [](const QmcWorkspaceU64& self) { return self.max_n; })
      .def_property_readonly("max_m", [](const QmcWorkspaceU64& self) { return self.max_m; })
      .def(
          "coalesce_coo_u64_f64_inplace_device",
          &QmcWorkspaceU64::coalesce_coo_u64_f64_inplace_device,
          py::arg("key_in"),
          py::arg("val_in"),
          py::arg("key_out"),
          py::arg("val_out"),
          ...
          py::arg("sync") = true)
      .def(
          "phi_pivot_resample_u64_f64_inplace_device",
          &QmcWorkspaceU64::phi_pivot_resample_u64_f64_inplace_device,
          py::arg("key_in"),
          py::arg("val_in"),
          py::arg("key_out"),
          py::arg("val_out"),
      ...
      .def(
          ...
          py::arg("n_in"),
	          ...
	          py::arg("sync") = true)
      .def(
          "sparse_dot_many_sorted_i32_f64_inplace_device",
          &QmcWorkspace::sparse_dot_many_sorted_i32_f64_inplace_device,
          py::arg("a_idx"),
          py::arg("a_val"),
          py::arg("a_nnz"),
          py::arg("b_idx"),
          ...
          py::arg("sync") = true)
	      .def(
	          "fciqmc_step_shift_i32_f64_inplace_device",
	          &QmcWorkspace::fciqmc_step_shift_i32_f64_inplace_device,
	          py::arg("drt"),
	          py::arg("state"),
	          py::arg("x_idx"),
	          py::arg("x_val"),
	          ...
	          py::arg("nspawn_two") = 0,
	          py::arg("seed_spawn") = 0,
	          py::arg("threads_spawn") = 128,
	          py::arg("threads_qmc") = 256,
	          py::arg("stream") = 0,
	          py::arg("sync") = true);
  ...
  m.def(
      "cas36_diag_guess_candidates_u64_dense_inplace_device",
      &cas36_diag_guess_candidates_u64_dense_inplace_device,
      py::arg("drt"),
      py::arg("ncsf"),
      py::arg("cand_idx_u64"),
  ...
  m.def(
      "cas36_cipsi_score_pt2_compact_u64_inplace_device",
      &cas36_cipsi_score_pt2_compact_u64_inplace_device,
      py::arg("idx_u64"),
      py::arg("vals_root_major"),
      py::arg("e_var"),
      py::arg("cand_hdiag"),
      py::arg("selected_idx_sorted_u64"),
      py::arg("denom_floor"),
  ...
  m.def(
      ...
      py::arg("hash_vals"),
      py::arg("label_lo") = 0,
      py::arg("label_hi") = std::numeric_limits<uint64_t>::max(),
      py::arg("selected_idx_sorted_u64") = py::none(),
      py::arg("target_mode") = 0,
      py::arg("overflow"),
      py::arg("sym_pq_allowed") = py::none(),
      py::arg("threads") = 256,
      py::arg("stream") = 0,
      py::arg("sync") = true);
  ...
  m.def(
      ...
      py::arg("out_hij"),
      py::arg("cap"),
      py::arg("membership_hash_keys") = py::none(),
      py::arg("membership_hash_cap") = 0,
      py::arg("out_diag") = py::none(),
      py::arg("out_n"),
      py::arg("overflow"),
      py::arg("threads") = 256,
      py::arg("stream") = 0,
  ...
  m.def(
      "cas36_sci_build_membership_hash_u64_inplace_device",
      &cas36_sci_build_membership_hash_u64_inplace_device,
      py::arg("sorted_keys"),
      py::arg("nkeys"),
      py::arg("hash_keys"),
  ...
  m.def(
      "pairwise_materialize_u64_inplace_device",
      &pairwise_materialize_u64_inplace_device,
      py::arg("drt"),
      py::arg("ncsf"),
      py::arg("sel_idx_u64"),
  ...
  m.def(
      "pairwise_hij_u64_inplace_device",
      &pairwise_hij_u64_inplace_device,
      py::arg("drt"),
      py::arg("ncsf"),
      py::arg("sel_idx_u64"),
