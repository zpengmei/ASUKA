...
from .sparse import sparse_abs_l1_on_support, sparse_dot_sorted, sparse_lookup_value
...
def choose_reference_index(
    ...
    *,
    ...
    x_idx_arr = np.asarray(x_idx).ravel()
    x_val_f64 = np.asarray(x_val, dtype=np.float64).ravel()
    if x_idx_arr.size != x_val_f64.size:
        raise ValueError("x_idx and x_val must have the same size")
    if x_idx_arr.size == 0:
        raise ValueError("x is empty")

    if preferred is not None:
        x_ref = sparse_lookup_value(x_idx_arr, x_val_f64, int(preferred))
        if x_ref != 0.0:
...
def projected_energy_ref_status(
    ...
    *,
    ...
    x_idx_i32 = np.asarray(x_idx).ravel()
    x_val_f64 = np.asarray(x_val, dtype=np.float64).ravel()
    if x_idx_i32.size != x_val_f64.size:
        raise ValueError("x_idx and x_val must have the same size")

    den = sparse_lookup_value(x_idx_i32, x_val_f64, ref_idx_i32)
    if den == 0.0:
...
def projected_energy_ref(
    ...
    *,
    ...
    ref_idx = int(ref_idx)
    x_idx = np.asarray(x_idx).ravel()
    x_val = np.asarray(x_val, dtype=np.float64).ravel()
    if x_idx.size != x_val.size:
        raise ValueError("x_idx and x_val must have the same size")

    x_ref = sparse_lookup_value(x_idx, x_val, ref_idx)
    if x_ref == 0.0:
