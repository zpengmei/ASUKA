...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit("pybind11 is required to build the orbitals CUDA extension") from e
    ...
    src_dir = os.path.join(repo_root, "asuka", "orbitals", "cuda_ext")
    cmake_build_dir = os.path.join(repo_root, "build", "orbitals_cuda_ext")
    ...
    if os.path.exists(cache_file):
        try:
            cache_txt = Path(cache_file).read_text(encoding="utf-8")
        except OSError:
            cache_txt = ""
        cached = None
    ...
    if shutil.which("ninja"):
        cmake_args += ["-G", "Ninja"]
        if os.path.exists(os.path.join(cmake_build_dir, "Makefile")):
            shutil.rmtree(cmake_build_dir, ignore_errors=True)
            os.makedirs(cmake_build_dir, exist_ok=True)
    ...
    build_args = ["cmake", "--build", cmake_build_dir, "-j", str(n_jobs)]
