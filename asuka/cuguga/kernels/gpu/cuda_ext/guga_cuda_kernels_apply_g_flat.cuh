...
// =============================================================================
// CIPSI frontier-hash variant: same segment walk, but accumulate into a hash map
// instead of a dense y vector. This is used to build sparse H|x> frontiers on
// GPU without materializing y[ncsf].
// =============================================================================

template <int MAX_NORB_T>
__global__ __launch_bounds__(256, 2)
void guga_apply_g_flat_scatter_atomic_frontier_hash_kernel_t(
    const int32_t* __restrict__ child,
    const int16_t* __restrict__ node_twos,
    const int64_t* __restrict__ child_prefix,
    const int8_t* __restrict__ steps_table,   // [ncsf,norb]
  ...
  // NOTE: Diagonal (p==q) contribution only ever targets csf_idx itself; in the
  // CIPSI external frontier build, csf_idx is always in the selected support.
  // Skipping it reduces hash pressure without affecting external amplitudes.
...
extern "C" void guga_apply_g_flat_scatter_atomic_frontier_hash_launch_stream(
  ...
  if (norb <= 16) {
    guga_apply_g_flat_scatter_atomic_frontier_hash_kernel_t<16><<<blocks, threads, 0, stream>>>(
        child,
        node_twos,
        child_prefix,
        steps_table,
