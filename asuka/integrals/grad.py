...
def compute_df_gradient_contributions_analytic_from_bar_x_packed_bases(
    ao_basis,
    aux_basis,
    *,
    atom_coords_bohr: np.ndarray,
    ...
    """Contract analytic DF 3c derivatives from an explicit unwhitened adjoint.

    This helper is for lanes that are naturally expressed as an adjoint of the
    unwhitened 3-center tensor ``X(mu,nu,Q) = (mu nu | Q)`` rather than the
    whitened DF factors ``B``. The Molcas SA-CASSCF/CASPT2 active lane falls
    into this category after the ``Qv`` transform.

    Parameters
    ----------
    bar_X_ao
        Adjoint of the unwhitened 3-center DF tensor with shape
        ``(nao, nao, naux)``.
    """
    ...
    if backend_s == "cpu":
        try:
            ...
            raise RuntimeError("CPU ERI extension is required for analytic DF bar_X contraction on backend='cpu'") from e
        ...
        raise NotImplementedError("analytic DF bar_X contraction is currently implemented only for backend='cpu'")
