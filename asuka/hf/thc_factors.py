...
def _make_grid_request(grid_kind: str, grid_spec: Any, grid_options: dict[str, Any] | None, *, stream: Any):
    """Create a GridRequest for THC grid generation (GPU-first)."""

    from asuka.density import GridRequest  # noqa: PLC0415

    kind_s = str(grid_kind).strip().lower()
    if kind_s in {"becke", "becke_grid", "atom", "atom-centered"}:
        kind = "becke"
        angular_prune = bool((grid_options or {}).get("angular_prune", False))
    elif kind_s in {"rdvr", "r-dvr", "r_dvr"}:
        kind = "rdvr"
        angular_prune = bool((grid_options or {}).get("angular_prune", True))
    elif kind_s in {"fdvr", "f-dvr", "f_dvr"}:
        kind = "fdvr"
        angular_prune = False
    else:
        raise ValueError("grid_kind must be one of: 'becke', 'rdvr', 'fdvr'")

    opts = {} if grid_options is None else dict(grid_options)
    return GridRequest(
        ...
        block_size=int(getattr(grid_spec, "block_size", 20_000)),
        prune_tol=float(getattr(grid_spec, "prune_tol", 1e-16)),
        threads=int(getattr(grid_spec, "threads", 256)),
        stream=stream,
        radial_rmax=float(opts.get("radial_rmax", getattr(grid_spec, "rmax", 20.0))),
        ortho_cutoff=float(opts.get("ortho_cutoff", 1e-10)),
        angular_prune=bool(angular_prune),
        atom_Z=opts.get("atom_Z", None),
        validate=bool(opts.get("validate", True)),
        overlap_max_abs_tol=float(opts.get("overlap_max_abs_tol", 1e-3)),
        max_sweeps=int(opts.get("max_sweeps", 50)),
        tol=float(opts.get("tol", 1e-12)),
    )


def _downselect_by_point_atom(cp, points, weights, point_atom, *, npt: int):
    """Per-atom downselect that preserves atom coverage."""

    points = cp.asarray(points, dtype=cp.float64)
    weights = cp.asarray(weights, dtype=cp.float64).ravel()
    point_atom = cp.asarray(point_atom, dtype=cp.int32).ravel()
    if int(point_atom.size) != int(weights.size):
        raise ValueError("point_atom shape mismatch")

    atoms = cp.unique(point_atom)
    natm = int(atoms.size)
    if natm <= 0:
        return cp.zeros((0, 3), dtype=cp.float64), cp.zeros((0,), dtype=cp.float64), cp.zeros((0,), dtype=cp.int32)
    ...
    base = int(npt // natm)
    rem = int(npt - base * natm)

    pts_out: list[Any] = []
    w_out: list[Any] = []
    a_out: list[Any] = []
    for rank, ia in enumerate(cp.asnumpy(atoms).tolist()):
        mask = point_atom == int(ia)
        pts_i = points[mask]
        w_i = weights[mask]
        ni = base + (1 if int(rank) < int(rem) else 0)
        if int(ni) <= 0:
            continue
        pts_j, w_j = _downselect_by_weight(cp, pts_i, w_i, npt=int(ni))
        pts_out.append(pts_j)
        w_out.append(w_j)
        a_out.append(cp.full((int(w_j.size),), int(ia), dtype=cp.int32))

    if len(pts_out) == 0:
        return cp.zeros((0, 3), dtype=cp.float64), cp.zeros((0,), dtype=cp.float64), cp.zeros((0,), dtype=cp.int32)

    return (
        cp.ascontiguousarray(cp.concatenate(pts_out, axis=0)),
        cp.ascontiguousarray(cp.concatenate(w_out, axis=0).ravel()),
        cp.ascontiguousarray(cp.concatenate(a_out, axis=0).ravel()),
    )
...
def build_thc_factors(
    ...
    *,
    ...
    from asuka.density import collect_grid  # noqa: PLC0415
    from asuka.density.grids import _coords_bohr  # noqa: PLC0415

    req = _make_grid_request(grid_kind_s, grid_spec, grid_options, stream=stream)
    if str(req.kind) == "rdvr" and dvr_basis is None:
        raise ValueError("grid_kind='rdvr' requires dvr_basis (use aux_basis or provide a separate DVR basis)")
    if str(req.kind) == "fdvr" and dvr_basis is None:
        raise ValueError("grid_kind='fdvr' requires dvr_basis (use aux_basis or provide a separate DVR basis)")

    grid = collect_grid(mol_or_coords, request=req, dvr_basis=dvr_basis)

    pts_all = cp.ascontiguousarray(cp.asarray(grid.points, dtype=cp.float64))
    w_all = cp.ascontiguousarray(cp.asarray(grid.weights, dtype=cp.float64).ravel())
    grid_npt_full_override = int(w_all.shape[0])

    if grid.point_atom is not None:
        point_atom = cp.ascontiguousarray(cp.asarray(grid.point_atom, dtype=cp.int32).ravel())
    elif str(req.kind) in {"becke", "rdvr"} and int(w_all.size) == 0:
        # Extremely pruned/empty atom-centered grids: keep the old convention of
        # providing an empty point_atom array rather than omitting it entirely.
        point_atom = cp.zeros((0,), dtype=cp.int32)

    # If the caller requests a reduced point count for atom-centered grids,
    # avoid a global weight-based downselect that can starve some atoms.
    if str(req.kind) == "becke" and point_atom is not None and npt is not None:
        npt_req = int(npt)
        if npt_req <= 0:
            raise ValueError("npt must be > 0")
        downselected = int(npt_req) < int(grid_npt_full_override)
        pts_all, w_all, point_atom = _downselect_by_point_atom(cp, pts_all, w_all, point_atom, npt=int(npt_req))
        npt = None

    elif str(req.kind) == "rdvr" and point_atom is not None and npt is not None:
        npt_req = int(npt)
        if npt_req <= 0:
            raise ValueError("npt must be > 0")
        downselected = int(npt_req) < int(grid_npt_full_override)
        ...
        pts_atoms = [pts_all[point_atom == int(ia)] for ia in range(natm)]
        w_atoms = [w_all[point_atom == int(ia)] for ia in range(natm)]
        ...
        if point_select in pivot_keys:
            try:
                ...
                # For fit-metric solves, also enforce k_i >= naux_i to avoid local rank deficiency.
        ...
        if npt is not None and point_select not in pivot_keys:
            base = int(npt_req // natm) if natm > 0 else 0
            rem = int(npt_req - base * natm) if natm > 0 else 0
