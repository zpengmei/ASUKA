...
m.def(
  ...
  m.def(
      ...
      [](py::object spAB_arr,
         ...
         uint64_t stream_ptr,
         bool sync) {
        require_threads_multiple_of_32(threads, "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qmn_inplace_device");
        if (threads > 256) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qmn_inplace_device requires threads <= 256");
        }
        if (nao <= 0 || naux <= 0 || nao_sph <= 0) throw std::invalid_argument("nao/naux/nao_sph must be > 0");
        if (la < 0 || lb < 0 || lc < 0) throw std::invalid_argument("la/lb/lc must be >= 0");
        if (la > 5 || lb > 5 || lc > 5) throw std::invalid_argument("supports only l<=5");
        ...
        auto pyv = cuda_array_view_from_object(pair_Py, "pair_Py");
	        ...
	        const bool bar_is_f4 = typestr_matches(bar, "<f4");
	        if (!bar_is_f8 && !bar_is_f4) {
	          throw std::invalid_argument("bar_X_sph_Qmn must have typestr <f8 or <f4 (got " + bar.typestr + ")");
	        }
	        auto a0s = cuda_array_view_from_object(shell_ao_start_sph, "shell_ao_start_sph");
	        auto sh_atom = cuda_array_view_from_object(shell_atom, "shell_atom");
	        auto grad_v = cuda_array_view_from_object(grad_dev, "grad_dev");
        ...
        if (bar_len != expected_bar) {
          throw std::invalid_argument("bar_X_sph_Qmn must have shape (naux*nao_sph*nao_sph,)");
	        ...
	        if (bar_is_f8) {
	          throw_on_cuda_error(
	              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qmn_launch_stream(
	                  static_cast<const int32_t*>(ab_arr.ptr),
	                  static_cast<int>(n_spAB),
	                  static_cast<const int32_t*>(cd.ptr),
	                  static_cast<int>(ntasks),
	          ...
	          throw_on_cuda_error(
	              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qmn_f32bar_launch_stream(
	                  static_cast<const int32_t*>(ab_arr.ptr),
	                  static_cast<int>(n_spAB),
	                  static_cast<const int32_t*>(cd.ptr),
	                  static_cast<int>(ntasks),
  ...
  // Packed-Qp spherical bar_X adjoint in AO-pair ("s2") layout.
  m.def(
      ...
      [](py::object spAB_arr,
        ...
        require_threads_multiple_of_32(
            threads, "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_inplace_device");
        if (threads > 256) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_inplace_device requires threads <= 256");
        }
        if (nao <= 0 || naux <= 0 || nao_sph <= 0) throw std::invalid_argument("nao/naux/nao_sph must be > 0");
        if (la < 0 || lb < 0 || lc < 0) throw std::invalid_argument("la/lb/lc must be >= 0");
        if (la > 5 || lb > 5 || lc > 5) throw std::invalid_argument("supports only l<=5");
        ...
        auto bar = cuda_array_view_from_object(bar_X_sph_Qp, "bar_X_sph_Qp");
        auto a0s = cuda_array_view_from_object(shell_ao_start_sph, "shell_ao_start_sph");
        auto sh_atom = cuda_array_view_from_object(shell_atom, "shell_atom");
        auto grad_v = cuda_array_view_from_object(grad_dev, "grad_dev");
        ...
        const bool bar_is_f4 = typestr_matches(bar, "<f4");
        if (!bar_is_f8 && !bar_is_f4) {
          throw std::invalid_argument("bar_X_sph_Qp must have typestr <f8 or <f4 (got " + bar.typestr + ")");
        }
        require_typestr(a0s, "shell_ao_start_sph", "<i4");
        require_typestr(sh_atom, "shell_atom", "<i4");
        require_typestr(grad_v, "grad_dev", "<f8");
        ...
        const int64_t bar_len = require_1d(bar, "bar_X_sph_Qp");
        const int64_t expected_bar = static_cast<int64_t>(naux) * ntri;
        if (bar_len != expected_bar) {
          throw std::invalid_argument("bar_X_sph_Qp must have shape (naux*ntri,) where ntri=nao_sph*(nao_sph+1)/2");
        }

        cudaStream_t stream = stream_from_uint(stream_ptr);
        if (bar_is_f8) {
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
          ...
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_f32bar_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
  ...
  m.def(
      ...
      [](py::object spAB_arr,
         ...
         bool sync) {
        require_threads_multiple_of_32(
            threads, "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_inplace_device");
        if (threads > 256) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_inplace_device requires threads <= 256");
        }
        if (cd_tile < 1 || cd_tile > 16) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_inplace_device requires cd_tile in [1,16]");
        }
        if (nao <= 0 || naux <= 0 || nao_sph <= 0) throw std::invalid_argument("nao/naux/nao_sph must be > 0");
        if (la < 0 || lb < 0 || lc < 0) throw std::invalid_argument("la/lb/lc must be >= 0");
        if (la > 5 || lb > 5 || lc > 5) throw std::invalid_argument("supports only l<=5");
        ...
        if (bar_is_f8) {
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
          ...
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_f32bar_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
  ...
  m.def(
      ...
      [](py::object spAB_arr,
         ...
         int lb,
         int lc,
         py::object bar_X_sph_Qp0,
         py::object bar_X_sph_Qp1,
         py::object shell_ao_start_sph,
         py::object shell_atom,
         ...
         bool sync) {
        require_threads_multiple_of_32(
            threads, "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_multibar2_inplace_device");
        if (threads > 256) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_multibar2_inplace_device requires threads <= 256");
        }
        if (nao <= 0 || naux <= 0 || nao_sph <= 0) throw std::invalid_argument("nao/naux/nao_sph must be > 0");
        if (la < 0 || lb < 0 || lc < 0) throw std::invalid_argument("la/lb/lc must be >= 0");
        if (la > 5 || lb > 5 || lc > 5) throw std::invalid_argument("supports only l<=5");
        if (grad_stride <= 0) throw std::invalid_argument("grad_stride must be > 0");
        ...
        auto bar0 = cuda_array_view_from_object(bar_X_sph_Qp0, "bar_X_sph_Qp0");
        auto bar1 = cuda_array_view_from_object(bar_X_sph_Qp1, "bar_X_sph_Qp1");
        auto a0s = cuda_array_view_from_object(shell_ao_start_sph, "shell_ao_start_sph");
        auto sh_atom = cuda_array_view_from_object(shell_atom, "shell_atom");
        ...
        const bool bar0_is_f8 = typestr_matches(bar0, "<f8");
        const bool bar0_is_f4 = typestr_matches(bar0, "<f4");
        const bool bar1_is_f8 = typestr_matches(bar1, "<f8");
        const bool bar1_is_f4 = typestr_matches(bar1, "<f4");
        if ((!bar0_is_f8 && !bar0_is_f4) || (!bar1_is_f8 && !bar1_is_f4)) {
          throw std::invalid_argument("bar_X_sph_Qp0/bar_X_sph_Qp1 must have typestr <f8 or <f4");
        }
        if (bar0.typestr != bar1.typestr) {
          throw std::invalid_argument(
              "bar_X_sph_Qp0 and bar_X_sph_Qp1 must have identical typestr (got " + bar0.typestr + " and " +
              bar1.typestr + ")");
        }
        require_typestr(a0s, "shell_ao_start_sph", "<i4");
        require_typestr(sh_atom, "shell_atom", "<i4");
        ...
        const int64_t ntri = static_cast<int64_t>(nao_sph) * static_cast<int64_t>(nao_sph + 1) / 2;
        const int64_t expected_bar = static_cast<int64_t>(naux) * ntri;
        if (require_1d(bar0, "bar_X_sph_Qp0") != expected_bar || require_1d(bar1, "bar_X_sph_Qp1") != expected_bar) {
          throw std::invalid_argument("bar_X_sph_Qp0/bar_X_sph_Qp1 must have shape (naux*ntri,)");
        }

        const int64_t grad_len = require_1d(grad_v, "grad_dev_multi");
        const int64_t expected_grad = static_cast<int64_t>(2) * static_cast<int64_t>(grad_stride);
        if (grad_len != expected_grad) {
          throw std::invalid_argument("grad_dev_multi must have shape (2*grad_stride,) for multibar2");
        }

        cudaStream_t stream = stream_from_uint(stream_ptr);
        if (bar0_is_f8) {
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_multibar2_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
                  ...
                  static_cast<int>(lb),
                  static_cast<int>(lc),
                  static_cast<const double*>(bar0.ptr),
                  static_cast<const double*>(bar1.ptr),
                  static_cast<const int32_t*>(a0s.ptr),
                  static_cast<const int32_t*>(sh_atom.ptr),
                  static_cast<double*>(grad_v.ptr),
          ...
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_multibar2_f32bar_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
                  ...
                  static_cast<int>(lb),
                  static_cast<int>(lc),
                  static_cast<const float*>(bar0.ptr),
                  static_cast<const float*>(bar1.ptr),
                  static_cast<const int32_t*>(a0s.ptr),
                  static_cast<const int32_t*>(sh_atom.ptr),
                  static_cast<double*>(grad_v.ptr),
                  static_cast<int64_t>(grad_stride),
      ...
      py::arg("lb"),
      py::arg("lc"),
      py::arg("bar_X_sph_Qp0"),
      py::arg("bar_X_sph_Qp1"),
      py::arg("shell_ao_start_sph"),
      py::arg("shell_atom"),
  ...
  m.def(
      ...
      [](py::object spAB_arr,
         ...
         bool sync) {
        require_threads_multiple_of_32(
            threads, "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar2_inplace_device");
        if (threads > 256) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar2_inplace_device requires threads <= 256");
        }
        if (cd_tile < 1 || cd_tile > 16) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar2_inplace_device requires cd_tile in [1,16]");
        }
        if (nao <= 0 || naux <= 0 || nao_sph <= 0) throw std::invalid_argument("nao/naux/nao_sph must be > 0");
        if (la < 0 || lb < 0 || lc < 0) throw std::invalid_argument("la/lb/lc must be >= 0");
        if (la > 5 || lb > 5 || lc > 5) throw std::invalid_argument("supports only l<=5");
        ...
        if (bar0_is_f8) {
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar2_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
          ...
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar2_f32bar_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
  ...
  m.def(
      ...
      [](py::object spAB_arr,
         ...
         bool sync) {
        require_threads_multiple_of_32(
            threads, "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_multibar3_inplace_device");
        if (threads > 256) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_multibar3_inplace_device requires threads <= 256");
        }
        if (nao <= 0 || naux <= 0 || nao_sph <= 0) throw std::invalid_argument("nao/naux/nao_sph must be > 0");
        if (la < 0 || lb < 0 || lc < 0) throw std::invalid_argument("la/lb/lc must be >= 0");
        if (la > 5 || lb > 5 || lc > 5) throw std::invalid_argument("supports only l<=5");
        ...
        auto bar0 = cuda_array_view_from_object(bar_X_sph_Qp0, "bar_X_sph_Qp0");
        auto bar1 = cuda_array_view_from_object(bar_X_sph_Qp1, "bar_X_sph_Qp1");
        auto bar2 = cuda_array_view_from_object(bar_X_sph_Qp2, "bar_X_sph_Qp2");
        auto a0s = cuda_array_view_from_object(shell_ao_start_sph, "shell_ao_start_sph");
        auto sh_atom = cuda_array_view_from_object(shell_atom, "shell_atom");
        auto grad_v = cuda_array_view_from_object(grad_dev_multi, "grad_dev_multi");
        ...
        const bool bar1_is_f8 = typestr_matches(bar1, "<f8");
        const bool bar1_is_f4 = typestr_matches(bar1, "<f4");
        const bool bar2_is_f8 = typestr_matches(bar2, "<f8");
        const bool bar2_is_f4 = typestr_matches(bar2, "<f4");
        if ((!bar0_is_f8 && !bar0_is_f4) || (!bar1_is_f8 && !bar1_is_f4) || (!bar2_is_f8 && !bar2_is_f4)) {
          throw std::invalid_argument("bar_X_sph_Qp0/bar_X_sph_Qp1/bar_X_sph_Qp2 must have typestr <f8 or <f4");
        }
        if (bar0.typestr != bar1.typestr || bar0.typestr != bar2.typestr) {
          throw std::invalid_argument("bar_X_sph_Qp0/bar_X_sph_Qp1/bar_X_sph_Qp2 must have identical typestr");
        }
        require_typestr(a0s, "shell_ao_start_sph", "<i4");
        require_typestr(sh_atom, "shell_atom", "<i4");
        require_typestr(grad_v, "grad_dev_multi", "<f8");
        ...
        const int64_t expected_bar = static_cast<int64_t>(naux) * ntri;
        if (require_1d(bar0, "bar_X_sph_Qp0") != expected_bar || require_1d(bar1, "bar_X_sph_Qp1") != expected_bar ||
            require_1d(bar2, "bar_X_sph_Qp2") != expected_bar) {
          throw std::invalid_argument("bar_X_sph_Qp0/bar_X_sph_Qp1/bar_X_sph_Qp2 must have shape (naux*ntri,)");
        }

        const int64_t grad_len = require_1d(grad_v, "grad_dev_multi");
        const int64_t expected_grad = static_cast<int64_t>(3) * static_cast<int64_t>(grad_stride);
        if (grad_len != expected_grad) {
          throw std::invalid_argument("grad_dev_multi must have shape (3*grad_stride,) for multibar3");
        }

        cudaStream_t stream = stream_from_uint(stream_ptr);
        if (bar0_is_f8) {
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_multibar3_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
                  ...
                  static_cast<const double*>(bar0.ptr),
                  static_cast<const double*>(bar1.ptr),
                  static_cast<const double*>(bar2.ptr),
                  static_cast<const int32_t*>(a0s.ptr),
                  static_cast<const int32_t*>(sh_atom.ptr),
                  static_cast<double*>(grad_v.ptr),
                  static_cast<int64_t>(grad_stride),
          ...
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_multibar3_f32bar_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
                  ...
                  static_cast<const float*>(bar0.ptr),
                  static_cast<const float*>(bar1.ptr),
                  static_cast<const float*>(bar2.ptr),
                  static_cast<const int32_t*>(a0s.ptr),
                  static_cast<const int32_t*>(sh_atom.ptr),
                  static_cast<double*>(grad_v.ptr),
                  static_cast<int64_t>(grad_stride),
  ...
  m.def(
      ...
      [](py::object spAB_arr,
         ...
         bool sync) {
        require_threads_multiple_of_32(
            threads, "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar3_inplace_device");
        if (threads > 256) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar3_inplace_device requires threads <= 256");
        }
        if (cd_tile < 1 || cd_tile > 16) {
          throw std::invalid_argument(
              "df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar3_inplace_device requires cd_tile in [1,16]");
        }
        if (nao <= 0 || naux <= 0 || nao_sph <= 0) throw std::invalid_argument("nao/naux/nao_sph must be > 0");
        if (la < 0 || lb < 0 || lc < 0) throw std::invalid_argument("la/lb/lc must be >= 0");
        if (la > 5 || lb > 5 || lc > 5) throw std::invalid_argument("supports only l<=5");
        ...
        if (bar0_is_f8) {
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar3_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
          ...
          throw_on_cuda_error(
              cueri_df_int3c2e_deriv_contracted_cart_allsp_atomgrad_sphbar_qp_abtile_multibar3_f32bar_launch_stream(
                  static_cast<const int32_t*>(ab_arr.ptr),
                  static_cast<int>(n_spAB),
                  static_cast<const int32_t*>(cd.ptr),
                  static_cast<int>(ntasks),
