...
__global__ void KernelContractFockTilesOrdered(
  ...
  for (int64_t idx = static_cast<int64_t>(blockIdx.x) * static_cast<int64_t>(blockDim.x) + threadIdx.x;
    ...
    const double v_Dbd = alpha * v * D_bd;
    const double v_Dbc = alpha * v * D_bc;
    const double v_Dad = alpha * v * D_ad;
    const double v_Dac = alpha * v * D_ac;

    // Primary: eri[(a,b),(c,d)] → K[a,c] += v * D[b,d]
    atomicAdd(&F_mat[a * N + c], v_Dbd);
    if (cd_neq)           atomicAdd(&F_mat[a * N + d], v_Dbc);
    if (ab_neq)           atomicAdd(&F_mat[b * N + c], v_Dad);
    if (ab_neq && cd_neq) atomicAdd(&F_mat[b * N + d], v_Dac);

    // Bra-ket swap: eri[(c,d),(a,b)] → K[c,a] += v * D[d,b]
    if (spab != spcd) {
      atomicAdd(&F_mat[c * N + a], v_Dbd);
      if (ab_neq)           atomicAdd(&F_mat[c * N + b], v_Dad);
      if (cd_neq)           atomicAdd(&F_mat[d * N + a], v_Dbc);
      if (cd_neq && ab_neq) atomicAdd(&F_mat[d * N + b], v_Dac);
    }
  }
}
