...
def test_uhf_local_thc_matches_dense_hf_reasonably():
    ...
    from asuka.frontend.scf import run_uhf_dense, run_uhf_thc
    from asuka.density import DeviceGridSpec

    mol = Molecule.from_atoms(
        # Li atom (doublet) as a small, robust UHF sanity case.
        [("Li", (0.0, 0.0, 0.0))],
        unit="Bohr",
        charge=0,
        spin=1,
    ...
    lthc = run_uhf_thc(
        ...
        auxbasis="autoaux",
        thc_mode="local",
        # Force a single block over all atoms so local-THC matches the global construction.
