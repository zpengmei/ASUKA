...
def test_rhf_thc_smoke_matches_df_reasonably():
    ...
    mol = Molecule.from_atoms(
        ...
        spin=0,
        basis="sto-3g",
        cart=False,  # default spherical AOs
    )

    # Baseline DF-SCF (small basis; this should be fast).
    df = run_rhf_df(
        mol,
        basis="sto-3g",
        auxbasis="autoaux",
    ...
    # THC-SCF with small grid (keep Z small) + density-difference warmup.
    ...
    # With a modest point set, THC should be in the right ballpark.
    # (Tight tolerances are not appropriate for a smoke test.)
