...
def test_rhf_local_thc_rdvr_smoke_matches_df_reasonably():
    ...
    lthc = run_rhf_thc(
        ...
        auxbasis="autoaux",
        thc_mode="local",
        # Tiny blocks to keep the test runtime low.
        thc_local_config={"block_max_ao": 4, "aux_schwarz_thr": 0.0, "sec_overlap_thr": 0.0},
        thc_grid_spec=grid,
        thc_grid_kind="rdvr",
        thc_dvr_basis="aux",
        thc_npt=2000,
    ...
    err = float(abs(float(lthc.scf.e_tot) - float(df.scf.e_tot)))
    assert err < 1e-1
