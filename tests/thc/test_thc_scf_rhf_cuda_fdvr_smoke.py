...
def test_rhf_thc_fdvr_smoke_matches_df_reasonably():
    ...
    # F-DVR is currently experimental and is validated against an overlap-quadrature
    # check. For typical small Gaussian DVR bases the approximate joint
    # diagonalization is not sufficiently accurate, so we expect an explicit error
    # rather than an unstable SCF divergence.
    with pytest.raises(RuntimeError, match="F-DVR grid failed overlap-quadrature validation"):
        _ = run_rhf_thc(
            mol,
            basis="sto-3g",
            auxbasis="autoaux",
            thc_grid_spec=grid,
            thc_grid_kind="fdvr",
            # Use a small DVR basis for this smoke test.
            thc_dvr_basis="sto-3g",
            thc_npt=None,
